# ============================================================
# Nexum TUI — English (en) Translation File
...
command-model-description = Open model selection panel (provider + real model + thinking); with args, switch to an available model from the active provider
...
statusbar-no-response-to-copy = no response to copy yet

copy-response-button = Copy
copy-response-copied = Copied
...
welcome-intro = Ready to help you develop, analyze, and coordinate technical tasks.
welcome-feature-code = Develop: write, refactor, and fix code
welcome-feature-explore = Explore: search the codebase and understand architecture
welcome-feature-document = Document: create reports, issues, and technical notes
welcome-feature-analyze = Analyze: review logs, errors, and code quality
welcome-feature-investigate = Investigate: search for information when the provider allows it
...
tip-2 = Ctrl+T switches available models, Ctrl+Shift+T switches provider
...
setup-welcome-title = Connect a provider
setup-choose-provider = Nexum needs a model to answer. Pick where it comes from.
setup-source-custom-api = Custom API
setup-source-migrate = Import existing config
setup-source-custom-desc = Manually enter provider details
setup-source-migrate-desc = Import config from ~/.claude/
setup-key-confirm = select
setup-key-select = move
setup-key-quit = skip
setup-configure-title = Configure your providers
setup-submit = Submit
setup-key-edit-submit = edit / save
setup-key-check = toggle
setup-key-back = back
...
setup-field-opus = Primary model
setup-field-sonnet = Balanced model
setup-field-haiku = Fast model
...
setup-complete-title = Done
setup-press-enter = Press
setup-to-start = to start using
setup-no-key = (no key)
setup-no-providers = No providers configured. Add one by selecting "Custom API" or importing an existing config.

setup-language-title = Choose your language
setup-language-prompt = Everything after this will be in the language you pick.
...
app-model-switched = Active model: { $alias } ({ $effort } effort)
...
login-field-opus-model = Primary model
login-field-sonnet-model = Balanced model
login-field-haiku-model = Fast model
...
render-user-answered = User answered Nexum's questions:
...
app-cli-no-input = No input prompt. Usage: nexum -p "your question" or echo "question" | nexum -p
