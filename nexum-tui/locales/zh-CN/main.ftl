# ============================================================
# Nexum TUI — 简体中文 (zh-CN) Translation File
...
command-model-description = 打开模型选择面板（Provider + 真实模型 + Thinking）；带参数时切换到当前 Provider 可用模型
...
statusbar-no-response-to-copy = 暂无回复可复制

copy-response-button = 复制
copy-response-copied = 已复制
...
welcome-intro = 准备好帮你开发、分析和协调技术任务。
welcome-feature-code = 开发：编写、重构和修复代码
welcome-feature-explore = 探索：搜索代码库并理解架构
welcome-feature-document = 文档：创建报告、issues 和技术笔记
welcome-feature-analyze = 分析：审查日志、错误和代码质量
welcome-feature-investigate = 调查：在 provider 允许时搜索信息
...
tip-2 = Ctrl+T 切换可用模型，Ctrl+Shift+T 切换 Provider
...
setup-welcome-title = 连接一个服务商
setup-choose-provider = Nexum 需要一个模型来回答。请选择来源。
setup-source-custom-api = Custom API
setup-source-migrate = 导入现有配置
setup-source-custom-desc = 手动输入 Provider 详情
setup-source-migrate-desc = 从 ~/.claude/ 导入配置
setup-key-confirm = 选择
setup-key-select = 移动
setup-key-quit = 跳过
setup-configure-title = 配置服务商
setup-submit = 提交
setup-key-edit-submit = 编辑 / 保存
setup-key-check = 勾选
setup-key-back = 返回
...
setup-field-opus = 主模型
setup-field-sonnet = 均衡模型
setup-field-haiku = 快速模型
...
setup-complete-title = 完成
setup-press-enter = 按
setup-to-start = 开始使用
setup-no-key = (无密钥)
setup-no-providers = 未配置任何 Provider，请选择"Custom API"或导入现有配置。

setup-language-title = 选择语言
setup-language-prompt = 之后的所有界面都会使用你选择的语言。
...
app-model-switched = 当前模型: { $alias } ({ $effort } effort)
...
login-field-opus-model = 主模型
login-field-sonnet-model = 均衡模型
login-field-haiku-model = 快速模型
...
render-user-answered = 用户回答了 Nexum 的问题：
...
app-cli-no-input = 无输入 prompt。用法: nexum -p "你的问题" 或 echo "问题" | nexum -p
