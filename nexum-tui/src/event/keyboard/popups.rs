    ...
    if matches!(
        ...
        match input {
            Input {
                ...
                super::super::mouse::copy_last_response_to_clipboard(app);
            }
            Input {
                key: Key::Char('q'),
                ctrl: true,
            ...
            // Esc: cerrar popup de preguntas (cancelar interacción)
            Input { key: Key::Esc, .. } => {
                app.session_mgr.current_mut().agent.interaction_prompt = None;
            }
