...
pub fn needs_setup(config: &crate::config::AppConfig) -> bool {
    // Env vars de providers de catálogo que disparan auto-login al arranque.
    // Si alguno está presente, el usuario ya tiene un provider configurable
    // (GLM Coding Plan) y no debe ser bloqueado por el wizard de first-run.
    const CATALOG_ENV_VARS: &[&str] = &["ZAI_CODING_API_KEY"];
    if CATALOG_ENV_VARS
        .iter()
        .any(|v| !std::env::var(v).unwrap_or_default().trim().is_empty())
    {
        return false;
    }
    // La pregunta que decide si es primer arranque: ¿hay ALGO usable?
    //
    // Antes se preguntaba lo otro —"¿a algún provider le falta credencial?"— y
    // eso ataba el wizard a la facturación de terceros: el día que OpenCode Zen
    // se quedó sin saldo perdió `free_access`, su key vacía volvió a contar como
    // "sin configurar", y el wizard reapareció sobre una instalación con cinco
    // providers andando. Que a uno de siete le falte credencial es normal.
    if nexum_acp::provider::catalog_has_usable_provider() {
        return false;
    }
    ...
    for provider in &config.providers {
        ...
        if provider.api_key.is_empty() {
            // Un provider de TIER LIBRE no tiene credencial por diseño: el
            // catálogo lo declara `free_access` y el runtime lo habilita sin
            // Authorization. Contarlo como "sin configurar" disparaba el wizard
            // de primer arranque sobre una instalación perfectamente
            // configurada — un bug disfrazado de primer arranque.
            //
            // Tercer lugar donde `api_key.is_empty()` significaba algo que no
            // es: los otros dos fueron LlmProvider::from_config y bearer_auth.
            if nexum_acp::provider::catalog_declares_free_access(&provider.id) {
                continue;
            }
...
fn handle_step_language(wizard: &mut SetupWizardPanel, input: Input) -> Option<SetupWizardAction> {
    ...
    match input {
        // El idioma se aplica al MOVER, no al confirmar: esta pantalla es la
        // única donde el usuario puede ver el efecto de su elección antes de
        // comprometerse. Antes la primera pantalla estaba en inglés y la
        // segunda en español, y no se sabía qué había cambiado qué.
        Input { key: Key::Up, .. } => {
            wizard.language_cursor =
                (wizard.language_cursor + LANGUAGE_OPTIONS.len() - 1) % LANGUAGE_OPTIONS.len();
            Some(SetupWizardAction::SetLanguage(
                LANGUAGE_OPTIONS[wizard.language_cursor].0.to_string(),
            ))
        }
        Input { key: Key::Down, .. } => {
...
/// 从 wizard 数据构建 NexumConfig（纯数据转换，无磁盘 I/O）
pub fn build_wizard_config(wizard: &SetupWizardPanel) -> crate::config::NexumConfig {
    let mut cfg = crate::config::NexumConfig::default();
...
pub fn save_setup_to(
    ...
    path: &std::path::Path,
) -> anyhow::Result<crate::config::NexumConfig> {
...
pub fn save_setup(wizard: &SetupWizardPanel) -> anyhow::Result<crate::config::NexumConfig> {
    let mut merged = crate::config::load().unwrap_or_else(|_| crate::config::NexumConfig::default());
