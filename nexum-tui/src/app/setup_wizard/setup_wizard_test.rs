// ALLOW justificado: temporales de test; verificado que cada uno lleva PID o UUID en el nombre. No hay recurso compartido entre
...
fn test_needs_setup_empty_providers_no_env() {
    // `needs_setup` pregunta al catálogo si hay algún provider usable, así que
    // sin aislar leería el de quien corra los tests.
    // Muta env global (API keys) y aísla el catálogo: un solo candado, tomado
    // una sola vez. `test_env_lock` es un Mutex de std, no reentrante.
...
fn test_needs_setup_empty_providers_but_env_key() {
    // `needs_setup` pregunta al catálogo si hay algún provider usable, así que
    // sin aislar leería el de quien corra los tests.
    // Un solo candado, tomado una sola vez: `test_env_lock` no es reentrante.
    let _guard = crate::ui::demo_mode::test_env_lock();
    let _catalogo = crate::app::catalog_fixture::CatalogoAislado::vacio();
    let config = AppConfig::default();
...
fn test_language_arrow_navigates() {
    ...
    let _ = handle_setup_wizard_key(&mut wizard, make_key(Key::Down));
    assert_eq!(wizard.language_cursor, 2);
    let _ = handle_setup_wizard_key(&mut wizard, make_key(Key::Down));
    assert_eq!(wizard.language_cursor, 0); // wraps around
    let _ = handle_setup_wizard_key(&mut wizard, make_key(Key::Up));
    assert_eq!(wizard.language_cursor, 2); // wraps around
}
