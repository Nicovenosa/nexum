use nexum_agent::messages::{BaseMessage, ContentBlock, MessageContent, ToolCallRequest};
use serde_json::json;
...
fn test_tool_args_cwd_consistency() {
    ...
    let msgs = vec![
        ...
        BaseMessage::Tool {
            id: nexum_agent::messages::MessageId::new(),
...
fn test_e2e_text_visible_between_tool_calls() {
    use nexum_agent::messages::{MessageContent, MessageId, ToolCallRequest};
...
fn make_agent_history(agent_count: usize, separated: bool) -> Vec<BaseMessage> {
    use nexum_agent::messages::MessageId;
...
// ─── Fix chat 2026-07-06: transcript inmutable ────────────────────────────────

/// Bug E2E real (MiMo/OpenCode Zen): el modelo streamea "Voy a comenzar..."
/// (visible), después llama TodoWrite/Glob, y el StateSnapshot llega con el
/// mensaje AI con content VACÍO (el texto viajó como reasoning_content).
/// Antes: set_completed() borraba current_ai_text y el rebuild descartaba la
/// burbuja → la respuesta visible DESAPARECÍA. Ahora se preserva como
/// mensaje AI sintético.
#[test]
fn test_texto_streameado_no_desaparece_con_snapshot_sin_texto() {
    let cwd = "/tmp/t";
    let mut pipeline = MessagePipeline::new(cwd.to_string());
    pipeline.begin_round();
    pipeline.push_chunk("Sí, puedo auditar el proyecto. Voy a comenzar por la arquitectura.");

    // ToolStart (TodoWrite) → finaliza el texto actual y registra el tool.
    let _ = pipeline.handle_event(AgentEvent::ToolStart {
        tool_call_id: "tc1".into(),
        name: "TodoWrite".into(),
        display: String::new(),
    ...
    // StateSnapshot SIN el texto visible: AI con content vacío + tool_calls
    // (así lo reporta el adapter OpenAI-compat cuando el texto fue reasoning).
    pipeline.set_completed(vec![
        BaseMessage::ai_with_tool_calls(
            MessageContent::text(""),
            vec![ToolCallRequest::new("tc1", "TodoWrite", json!({"todos": []}))],
        ),
        BaseMessage::Tool {
            id: nexum_agent::messages::MessageId::new(),
            tool_call_id: "tc1".to_string(),
    ...
    let all_text: String = tail
        .iter()
        .filter_map(|vm| vm.assistant_raw_text())
        .collect::<Vec<_>>()
        .join("\n");
    assert!(
        all_text.contains("Voy a comenzar por la arquitectura"),
        "el texto ya visto por el usuario NO puede desaparecer del transcript; tail: {all_text:?}"
    );
}

/// Caso normal: el snapshot SÍ trae el texto en el mensaje AI → no se
/// duplica (el sintético solo se agrega cuando falta).
#[test]
fn test_texto_streameado_no_se_duplica_cuando_snapshot_lo_trae() {
    let cwd = "/tmp/t";
    let mut pipeline = MessagePipeline::new(cwd.to_string());
    pipeline.begin_round();
    pipeline.push_chunk("Hola, arranco la auditoría ahora.");

    pipeline.set_completed(vec![BaseMessage::ai("Hola, arranco la auditoría ahora.")]);

    let tail = pipeline.build_tail_vms();
    let occurrences = tail
        .iter()
        .filter_map(|vm| vm.assistant_raw_text())
        .filter(|t| t.contains("arranco la auditoría"))
        .count();
    assert_eq!(occurrences, 1, "el texto no debe duplicarse: {tail:?}");
}

/// El texto preservado convive con el Todo/tool del mismo turno: la
/// respuesta inicial y el progreso coexisten (regla 1 del spec).
#[test]
fn test_respuesta_inicial_coexiste_con_tool_blocks() {
    let cwd = "/tmp/t";
    let mut pipeline = MessagePipeline::new(cwd.to_string());
    pipeline.begin_round();
    pipeline.push_chunk("Empiezo la auditoría, filosóficamente hablando.");
    let _ = pipeline.handle_event(AgentEvent::ToolStart {
    ...
    // Sin snapshot todavía: streaming bubble + tool pendiente.
    let tail = pipeline.build_tail_vms();
    let has_text = tail.iter().any(|vm| {
        vm.assistant_raw_text()
            .is_some_and(|t| t.contains("filosóficamente"))
    });
    // El tool puede renderizar como ToolBlock suelto o agrupado en
    // ToolCallGroup (aggregate_tool_groups) — ambos cuentan como visible.
    let has_tool = tail.iter().any(|vm| match vm {
        MessageViewModel::ToolBlock { tool_name, .. } => tool_name == "Glob",
        MessageViewModel::ToolCallGroup { tools, .. } => {
            tools.iter().any(|t| t.tool_name == "Glob")
        }
        _ => false,
    });
    assert!(has_text, "la respuesta visible sigue en el tail: {tail:?}");
    assert!(has_tool, "el tool block convive con la respuesta: {tail:?}");
}
