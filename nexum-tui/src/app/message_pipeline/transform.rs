...
impl MessagePipeline {
    ...
    pub fn build_streaming_bubble(&self) -> MessageViewModel {
        ...
        if !self.current_ai_reasoning.is_empty() {
            let reasoning =
                crate::ui::secret_redact::redact_secrets(&self.current_ai_reasoning);
            blocks.push(ContentBlockView::Reasoning {
                char_count: reasoning.chars().count(),
                text: reasoning,
                tail_lines: None,
            });
        }
        if !self.current_ai_text.trim().is_empty() {
            let text = crate::ui::secret_redact::redact_secrets(&self.current_ai_text);
    ...
    pub fn messages_to_view_models(msgs: &[BaseMessage], cwd: &str) -> Vec<MessageViewModel> {
        ...
        // Turn grouping: un solo header/footer de NEXUM por turno (restore).
        crate::ui::message_view::group_turn_cards(&mut vms);
        vms
    }
