...
use crate::ui::{
    message_view::{ContentBlockView, MessageViewModel},
    render_thread::RenderEvent,
};
...
impl App {
    ...
    pub(crate) fn render_rebuild(&self) {
        let mut vms = {
            ...
            self.resolve_render_vms(session)
        };
        Self::prepare_assistant_footers(&mut vms, self);
        let session = self.session_mgr.current();
        let _ = session
            .messages
            .render_tx
    ...
    pub(crate) fn render_rebuild_with_anchor(&self, anchor_message_idx: usize) {
        let focused = self.session_mgr.current().focused_instance_id.is_some();
        if focused {
            // 聚焦模式：从 SQLite 加载，忽略锚点
            let mut vms = {
                let session = self.session_mgr.current();
                self.resolve_render_vms(session)
            };
    ...
    /// 为 AssistantBubble 计算 footer metadata 并标记 copied 状态。
    ///
    /// 该函数在消息发往渲染线程前调用，只影响 UI-only 字段，不改变语义 hash。
    fn prepare_assistant_footers(vms: &mut Vec<MessageViewModel>, app: &App) {
        // 1. 计算每个 AssistantBubble 的 char_count。
        for vm in vms.iter_mut() {
            if let MessageViewModel::AssistantBubble { blocks, char_count, .. } = vm {
                *char_count = blocks
                    .iter()
        ...
        // 2. 找到最后一个有 texto 的 AssistantBubble。
        let last_text_idx = vms.iter().rposition(|vm| {
        ...
        let copy_until = app.session_mgr.current().ui.copy_message_until;
        let spinner = &app.session_mgr.current().spinner_state;
        ...
        {
            if copy_until.is_some_and(|until| until > now) {
                *copied_label_until = copy_until;
            }
            if spinner.mode() == &SpinnerMode::Idle {
                *elapsed_ms = spinner.last_summary_elapsed_ms();
                *token_count = spinner.displayed_tokens();
            }
        }
    }
}
