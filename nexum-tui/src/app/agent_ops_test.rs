// ─── ACP configuration bridge tests ─────────────────────────────────────

#[test]
fn test_config_option_update_extracts_host_model_state() {
    let options = super::acp_bridge::config_option_values(&serde_json::json!({
        "sessionUpdate": "config_option_update",
        "configOptions": [
            { "id": "model", "type": "select", "currentValue": "sonnet" },
            { "id": "thinking_effort", "type": "select", "currentValue": "high" },
            { "id": "mode", "type": "select", "currentValue": "bypass" }
        ]
    }));

    assert_eq!(options.get("model"), Some(&"sonnet".to_string()));
    assert_eq!(options.get("thinking_effort"), Some(&"high".to_string()));
    assert_eq!(options.get("mode"), Some(&"bypass".to_string()));
}

#[test]
fn test_config_option_update_applies_host_model_state_to_visual_config() {
    let mut config = crate::config::NexumConfig::default();
    config.config.active_provider_id = "fixture".to_string();
    config.config.providers = vec![crate::config::ProviderConfig {
        id: "fixture".to_string(),
        provider_type: "openai".to_string(),
        api_key: "fixture-key".to_string(),
        models: crate::config::ProviderModels {
            sonnet: "fixture-sonnet".to_string(),
            ..Default::default()
        },
        ..Default::default()
    }];
    let shared = std::sync::Arc::new(parking_lot::RwLock::new(config));
    let permission_mode = nexum_middlewares::prelude::SharedPermissionMode::new(
        nexum_middlewares::prelude::PermissionMode::Default,
    );
    let values = super::acp_bridge::config_option_values(&serde_json::json!({
        "sessionUpdate": "config_option_update",
        "configOptions": [
            { "id": "model", "type": "select", "currentValue": "sonnet" },
            { "id": "thinking_effort", "type": "select", "currentValue": "high" },
    ...
    let visual = super::acp_bridge::apply_config_option_values(&shared, &permission_mode, &values);

    assert_eq!(shared.read().config.active_alias, "sonnet");
    assert_eq!(
        shared
            .read()
            .config
    ...
    assert_eq!(
        visual,
        Some(("OpenAI".to_string(), "fixture-sonnet".to_string()))
    );
}
...
fn test_submit_message_records_round_start_vm_idx() {
    let mut messages = vec![
        ...
        crate::ui::message_view::MessageViewModel::from_base_message(
            &nexum_agent::messages::BaseMessage::ai("a1".to_string()),
            &[],
        ),
...
fn test_rebuildall_system_note_anchor_insertion() {
    ...
    let tail_vms = vec![MessageViewModel::from_base_message(
        &nexum_agent::messages::BaseMessage::ai("response".to_string()),
        &[],
    )];
...
fn test_multiple_rebuildalls_preserve_note_position() {
    ...
    let tail_vms_1 = vec![MessageViewModel::from_base_message(
        &nexum_agent::messages::BaseMessage::ai("response1".to_string()),
        &[],
    )];
    ...
    let tail_vms_2 = vec![MessageViewModel::from_base_message(
        &nexum_agent::messages::BaseMessage::ai("response2".to_string()),
        &[],
    )];
