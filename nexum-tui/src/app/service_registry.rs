...
use nexum_agent::interaction::ChannelState;
...
use crate::{config::NexumConfig, thread::ThreadStore};

/// `ServiceRegistry` 中共享的配置类型：单一来源（Single Source of Truth）。
///
/// TUI 与 ACP Server 共享同一个 `Arc<RwLock<NexumConfig>>`，写入会即时传播，
/// 无需手动调用 `sync_acp_config`。
pub type SharedPeriConfig = Arc<RwLock<NexumConfig>>;
...
pub struct ServiceRegistry {
    ...
    pub nexum_config: SharedPeriConfig,
    pub cwd: String,
    pub provider_name: String,
    pub model_name: String,
    pub permission_mode: Arc<SharedPermissionMode>,
    /// SPEC-SECURITY-001 (GAP-P0): YOLO/Bypass activo por override explícito.
    /// Solo estado (para banner/status/doctor); jamás contenido sensible.
    pub yolo_mode_active: bool,
