...
pub use global_ui_state::{AgentMode, ApprovalDisplayMode, GlobalUiState};
...
mod ui_state;
pub use ui_state::{CopyButtonHitbox, InputMutation, InputMutationSource, UiState};
...
use nexum_middlewares::prelude::HitlDecision;
...
impl App {
    pub async fn new() -> Self {
        ...
        let nexum_config = crate::config::load().ok();
        ...
        let provider_from_config = nexum_config
            .as_ref()
        ...
        let (provider_name, model_name, _status_msg) =
            match provider_from_config.or_else(agent::LlmProvider::from_env) {
                Some(p) => {
                    ...
                    let name = nexum_config
                        .as_ref()
                        .and_then(model_panel::provider_display_name)
        ...
        let thread_store: Arc<dyn ThreadStore> = match SqliteThreadStore::default_path().await {
            ...
            Err(_) => Arc::new(
                SqliteThreadStore::fallback_path()
                    .await
                    .expect("no se pudo abrir la base de threads"),
            ),
        };
        ...
        let skills = {
            ...
            let skill_roots =
                nexum_middlewares::skills::resolve_skill_roots(&cwd, vec![], disable_bundled);
            nexum_middlewares::skills::scan_skill_roots(&skill_roots)
        };

        let cron_state = CronState::default();

        let (bg_event_tx, bg_event_rx) = tokio::sync::mpsc::channel(128);

        let diff_enabled = nexum_config
            .as_ref()
            .map(|c| c.config.diff_enabled)
            .unwrap_or(false);
        let streaming_mode = nexum_config
            .as_ref()
        ...
        // SPEC-SECURITY-001 (GAP-P0): fail-safe. El default de App es HITL
        // activo; main.rs lo sobreescribe con el modo resuelto por CLI/env.
        let permission_mode = nexum_middlewares::prelude::SharedPermissionMode::new(
            nexum_middlewares::prelude::PermissionMode::Default,
        );
        let channel_state = nexum_agent::interaction::ChannelState::new();
        let services = ServiceRegistry {
            nexum_config: std::sync::Arc::new(parking_lot::RwLock::new(
                nexum_config.clone().unwrap_or_default(),
            )),
            cwd: cwd.clone(),
            provider_name: provider_name.clone(),
            model_name: model_name.clone(),
            permission_mode: permission_mode.clone(),
            yolo_mode_active: false,
    ...
    pub fn new_session(&mut self) {
        ...
        let plugin_skill_roots = self
            ...
            .unwrap_or_default();
        let disable_bundled = nexum_middlewares::skills::load_disable_bundled_skills();
        let skill_roots = nexum_middlewares::skills::resolve_skill_roots(
            &self.services.cwd,
            plugin_skill_roots,
    ...
    pub fn spawn_mcp_init(&mut self) {
        use nexum_middlewares::mcp::{McpClientPool, McpInitStatus};
        ...
        let oauth_cb: Box<dyn Fn(nexum_middlewares::mcp::OAuthFlowEvent) + Send + Sync> =
            Box::new(move |ev| {
                use nexum_middlewares::mcp::OAuthFlowEvent;
    ...
    pub fn set_loading(&mut self, loading: bool) {
        ...
        if loading {
            ...
            s.spinner_state
                .set_mode(nexum_widgets::SpinnerMode::Responding);
        } else {
            s.spinner_state.set_mode(nexum_widgets::SpinnerMode::Idle);
    ...
    ///
    /// 配置写入共享的 `Arc<RwLock<NexumConfig>>`，ACP Server 持有同一 `Arc`，
    /// 因此无需再调用 `sync_acp_config`。
    pub fn refresh_after_setup(&mut self, cfg: crate::config::NexumConfig) {
        *self.services.nexum_config.write() = cfg;
        let cfg_ref = self.services.nexum_config.read();
    ...
    pub fn get_compact_config(&self) -> nexum_agent::agent::CompactConfig {
        let mut config = self
            .services
            .nexum_config
            .read()
