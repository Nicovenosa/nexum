...
//!   polling.rs   — poll_agent, poll_background_events
...
impl App {
        ...
        match event {
            ...
            AgentEvent::SubagentLifecycle {
                ...
                if started {
                    ...
                    self.session_mgr
                        .current_mut()
                        .spinner_state
                        .set_mode(nexum_widgets::SpinnerMode::ToolUse);
                    self.session_mgr
                        .current_mut()
                        .spinner_state
                        .set_verb(Some(&verb));
            ...
            AgentEvent::TokenUsageUpdate {
                ...
                if let Some(provenance) = self
                    ...
                    .as_mut()
                {
                    provenance.record_llm_response(
                        &model,
                        usage.input_tokens,
                        usage.output_tokens,
                        Some(200),
            ...
            AgentEvent::AssistantChunk {
                ...
                if let Some(provenance) = self
                    ...
                    .as_mut()
                {
                    provenance.record_response_chunk();
                }
                // 跨切面：spinner
                self.session_mgr
                    .current_mut()
    ...
    // poll_agent/poll_background_events are in polling.rs
}
