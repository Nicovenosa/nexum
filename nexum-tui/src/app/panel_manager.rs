...
use super::{
    ...
    provider_panel::ProviderPanel, service_registry::ServiceRegistry,
...
impl PanelKind {
    ...
    pub fn mutex_group(&self) -> MutexGroup {
        match self {
            ...
            PanelKind::Status | PanelKind::Memory | PanelKind::Betas | PanelKind::Provider => {
                MutexGroup::Info
            }
    ...
    pub fn scope(&self) -> PanelScope {
        match self {
            ...
            | PanelKind::Tasks
            | PanelKind::Betas
            | PanelKind::Provider => PanelScope::Global,
        }
    }
}
...
macro_rules! panel_dispatch {
    ...
    (kind: $state:expr) => {
        match $state {
            ...
            PanelState::Provider(_) => PanelKind::Provider,
        }
    };
    ...
    (any:ref $state:expr) => {
        match $state {
            ...
            PanelState::Provider(p) => p as &dyn Any,
        }
    };
    ...
    (any:mut $state:expr) => {
        match $state {
            ...
            PanelState::Provider(p) => p as &mut dyn Any,
        }
    };
...
impl PanelContext<'_> {
    // `sync_acp_config` 已移除：TUI 与 ACP Server 共享同一 `Arc<RwLock<NexumConfig>>`，
    // 写入即时传播，无需手动同步。
}
