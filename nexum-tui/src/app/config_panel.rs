...
fn save_config_now(panel: &mut ConfigPanel, ctx: &mut PanelContext<'_>) {
    ...
    if panel.apply_edit(&mut cfg, &ctx.services.lc).is_ok() {
        if let Some(ref lang) = cfg.config.language.clone() {
            ...
            let _ = ctx.session_mgr.current().messages.render_tx.try_send(
                crate::ui::render_thread::RenderEvent::SetLocale(lang.clone()),
            );
            let _ = ctx.session_mgr.current().messages.render_tx.try_send(
                crate::ui::render_thread::RenderEvent::SetCopyButtonLabel(
                    ctx.services.lc.tr("copy-response-button").to_string(),
                ),
            );
        }
...
impl ConfigPanel {
    ...
    /// 可选语言列表："" (auto) → "en" → "zh-CN" → "es-AR" → ""
    const LANGUAGE_OPTIONS: &[&str] = &["en", "zh-CN", "es-AR"];
    ...
    pub fn apply_edit(
        ...
        let compact = cfg
            .config
            .compact
            .get_or_insert_with(nexum_agent::agent::CompactConfig::default);
