use super::*;
use crate::config::{NexumConfig, ProviderConfig, ProviderModels};

fn make_config(alias: &str, provider_id: &str, model_id: &str, provider_type: &str) -> NexumConfig {
    let mut cfg = NexumConfig::default();
...
#[test]
fn test_from_config_openai_active_alias_can_be_real_model() {
    let mut cfg = make_config("opus", "openai", "", "openai");
    cfg.config.active_alias = "deepseek-v4-flash".to_string();
    let provider = LlmProvider::from_config(&cfg).expect("modelo real openai debe ser ejecutable");
...
#[test]
fn test_from_config_for_alias_openai_unknown_alias_is_model_name() {
    let cfg = make_config("opus", "openai", "", "openai");
    let p = LlmProvider::from_config_for_alias(&cfg, "deepseek-v4-flash").unwrap();
    assert_eq!(p.display_name(), "OpenAI");
    assert_eq!(p.model_name(), "deepseek-v4-flash");
}
