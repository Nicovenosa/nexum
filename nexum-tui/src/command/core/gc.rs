use nexum_agent::messages::{BaseMessage, ContentBlock, MessageContent};
...
impl Command for GcCommand {
    ...
    fn execute(&self, app: &mut App, _args: &str) {
        ...
        // ── Markdown/Diff 缓存诊断 ──
        let md_cache_len = nexum_widgets::markdown::cache::MarkdownCache::global().len();
...
fn estimate_content_block_heap(b: &ContentBlock) -> usize {
    match b {
        ...
        ContentBlock::Image { source } => match source {
            nexum_agent::messages::ImageSource::Base64 { media_type, data } => {
                media_type.len() + data.len()
            }
            nexum_agent::messages::ImageSource::Url { url } => url.len(),
        },
        ContentBlock::Document { source, title } => {
            let src = match source {
                nexum_agent::messages::DocumentSource::Base64 { media_type, data } => {
                    media_type.len() + data.len()
                }
                nexum_agent::messages::DocumentSource::Url { url } => url.len(),
                nexum_agent::messages::DocumentSource::Text { text } => text.len(),
            };
