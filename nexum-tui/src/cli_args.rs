...
impl From<PluginScope> for nexum_middlewares::plugin::InstallScope {
    fn from(scope: PluginScope) -> Self {
        match scope {
            PluginScope::User => nexum_middlewares::plugin::InstallScope::User,
            PluginScope::Project => nexum_middlewares::plugin::InstallScope::Project,
            PluginScope::Local => nexum_middlewares::plugin::InstallScope::Local,
        }
    }
}
...
mod tests {
    ...
    fn test_plugin_scope_parse() {
        ...
        assert_eq!(
            nexum_middlewares::plugin::InstallScope::from(PluginScope::User),
            nexum_middlewares::plugin::InstallScope::User
        );
        assert_eq!(
            nexum_middlewares::plugin::InstallScope::from(PluginScope::Project),
            nexum_middlewares::plugin::InstallScope::Project
        );
        assert_eq!(
            nexum_middlewares::plugin::InstallScope::from(PluginScope::Local),
            nexum_middlewares::plugin::InstallScope::Local
        );
    }
}
