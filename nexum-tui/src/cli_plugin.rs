...
fn load_plugins() -> Vec<PluginListEntry> {
    ...
    let installed = nexum_middlewares::plugin::config::load_installed_plugins(Some(&plugins_path))
        .unwrap_or_default();
    ...
    installed
        ...
        .map(|p| PluginListEntry {
            ...
            enabled: true,
            scope: match p.scope {
                nexum_middlewares::plugin::InstallScope::User => "user",
                nexum_middlewares::plugin::InstallScope::Project => "project",
                nexum_middlewares::plugin::InstallScope::Local => "local",
            }
            .to_string(),
        })
        .collect()
...
pub async fn run_plugin_install(plugin_name: &str, scope_str: &str) -> Result<()> {
    ...
    let claude_dir = dirs_next::home_dir()
        .unwrap_or_else(|| std::path::PathBuf::from("."))
        .join(".claude");
    let cache_dir = nexum_middlewares::plugin::config::marketplaces_cache_dir();

    let (name, marketplace) = plugin_name
        .split_once('@')
        .unwrap_or((plugin_name, "claude-plugins-official"));

    let result = nexum_middlewares::plugin::install_plugin(
        name,
        marketplace,
        scope.into(),
        &cache_dir,
...
pub async fn run_plugin_uninstall(plugin_id: &str, _scope_str: Option<&str>) -> Result<()> {
    ...
    nexum_middlewares::plugin::uninstall_plugin(plugin_id, &claude_dir, None)
        .await
