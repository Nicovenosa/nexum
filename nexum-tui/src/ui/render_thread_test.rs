...
fn full_wrap(vms: &[MessageViewModel], width: u16) -> (usize, Vec<super::WrappedLineInfo>) {
    ...
    for vm in vms {
        let (mut lines, _button) = super::RenderTask::render_one(
            &mut vm.clone(),
            0,
            width as usize,
            false,
...
// ── Botón "📋 Copiar" en el cache (sprint copy-button 2026-07-08) ─────────

fn make_done_assistant(raw: &str) -> MessageViewModel {
    // assistant_blocks: is_streaming=false, show_header/footer=true (default).
    // Seteamos metadata realista para que el botón se renderice inline (diseño
    // minimalista 2026-07-08).
    let char_count = raw.chars().count();
    let mut vm = MessageViewModel::assistant_blocks(vec![crate::ui::message_view::ContentBlockView::Text {
        raw: raw.to_string(),
    ...
    if let MessageViewModel::AssistantBubble { char_count: c, elapsed_ms: e, .. } = &mut vm {
        *c = char_count;
        *e = 1234;
    }
    vm
...
#[tokio::test]
async fn test_cache_registra_copy_button_de_turno_completado() {
    let (tx, cache, _notify) = spawn_render_thread(80);
    let vms = vec![
        MessageViewModel::user("pregunta".to_string()),
        make_done_assistant("respuesta final"),
    ];
    tx.send(RenderEvent::Rebuild(vms)).await.unwrap();
    wait_render().await;

    let c = cache.read();
    assert_eq!(c.copy_buttons.len(), 1, "exactamente UN botón por turno");
    let btn = &c.copy_buttons[0];
    assert_eq!(btn.message_idx, 1, "el botón pertenece al AssistantBubble");
    assert!(
        btn.col_start > 10,
        "botón inline alineado a la derecha; col_start={}",
        btn.col_start
    );
    // La línea apuntada por line_idx contiene el label (locale default: en).
    let plain = &c.wrap_map[btn.line_idx].plain_text;
    assert!(
        plain.contains("📋 Copy"),
        "line_idx apunta a la línea del botón, no a otra: {plain}"
    );
}

#[tokio::test]
async fn test_boton_aparece_cuando_streaming_termina() {
    // Regresión del reuse "cosmético": el flip is_streaming true→false con
    // blocks idénticos debe RE-renderizar la burbuja para que aparezca el
    // botón (is_cosmetic_change ya no trata ese flip como cosmético).
    let (tx, cache, _notify) = spawn_render_thread(80);

    let mut streaming = make_done_assistant("misma respuesta");
    if let MessageViewModel::AssistantBubble { is_streaming, .. } = &mut streaming {
        *is_streaming = true;
    }
    streaming.recompute_hash();
    tx.send(RenderEvent::Rebuild(vec![streaming]))
        .await
        .unwrap();
    wait_render().await;
    assert!(
        cache.read().copy_buttons.is_empty(),
        "sin botón mientras el streaming está activo"
    );

    let done = make_done_assistant("misma respuesta");
    tx.send(RenderEvent::Rebuild(vec![done])).await.unwrap();
    wait_render().await;
    let c = cache.read();
    assert_eq!(
        c.copy_buttons.len(),
        1,
        "al terminar el streaming aparece el botón (blocks idénticos)"
    );
    let plain = &c.wrap_map[c.copy_buttons[0].line_idx].plain_text;
    assert!(
        plain.contains("📋 Copy"),
        "línea correcta: {plain}"
    );
}

#[tokio::test]
async fn test_burbujas_intermedias_no_registran_boton() {
    // Turno agrupado: burbuja intermedia (show_footer=false) + final.
    let (tx, cache, _notify) = spawn_render_thread(80);
    let mut mid = make_done_assistant("preámbulo");
    if let MessageViewModel::AssistantBubble { show_footer, .. } = &mut mid {
        *show_footer = false;
    }
    mid.recompute_hash();
    let last = make_done_assistant("respuesta final");
    tx.send(RenderEvent::Rebuild(vec![mid, last]))
        .await
        .unwrap();
    wait_render().await;
    ...
    assert_eq!(
        c.copy_buttons.len(),
        1,
        "solo la burbuja con show_footer registra botón (no duplicados)"
    );
    assert_eq!(c.copy_buttons[0].message_idx, 1);
}

#[tokio::test]
async fn test_clear_limpia_copy_buttons() {
    let (tx, cache, _notify) = spawn_render_thread(80);
    tx.send(RenderEvent::Rebuild(vec![make_done_assistant("r")]))
        .await
        .unwrap();
    wait_render().await;
    assert!(!cache.read().copy_buttons.is_empty());

    tx.send(RenderEvent::Clear).await.unwrap();
    wait_render().await;
    assert!(
        cache.read().copy_buttons.is_empty(),
        "Clear vacía los botones del cache"
    );
}

#[tokio::test]
async fn test_boton_sobrevive_reuso_de_prefijo_estable() {
    // El botón de un turno viejo debe seguir en el cache cuando un rebuild
    // posterior reusa el prefijo estable (hash sin cambios).
    let (tx, cache, _notify) = spawn_render_thread(80);
    let turno_viejo = vec![
        MessageViewModel::user("pregunta 1".to_string()),
        make_done_assistant("respuesta 1"),
    ];
    tx.send(RenderEvent::Rebuild(turno_viejo.clone()))
        .await
        .unwrap();
    wait_render().await;
    assert_eq!(cache.read().copy_buttons.len(), 1);

    // Nuevo turno: prefijo (turno viejo) estable + user nuevo + assistant nuevo.
    let mut vms = turno_viejo;
    vms.push(MessageViewModel::user("pregunta 2".to_string()));
    vms.push(make_done_assistant("respuesta 2"));
    tx.send(RenderEvent::Rebuild(vms)).await.unwrap();
    wait_render().await;
    ...
    assert_eq!(
        c.copy_buttons.len(),
        2,
        "un botón por turno completado, el viejo se preserva vía reuse"
    );
    assert_eq!(c.copy_buttons[0].message_idx, 1);
    assert_eq!(c.copy_buttons[1].message_idx, 3);
    // Ambos line_idx apuntan a líneas de botón reales.
    for btn in c.copy_buttons.iter() {
        assert!(
            c.wrap_map[btn.line_idx]
                .plain_text
                .contains("📋 Copy"),
            "line_idx desalineado tras dedup/reuse"
        );
    }
}
