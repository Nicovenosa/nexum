...
use nexum_agent::messages::{BaseMessage, ContentBlock};
...
pub(crate) fn build_diff_lines(
    ...
    let lines = nexum_widgets::diff::render_diff(&diff_input, 80, &nexum_widgets::DarkTheme);
    if lines.is_empty() {
        None
    } else {
        Some(lines)
    ...
    pub fn from_base_message_with_cwd(
        ...
        match msg {
            BaseMessage::Human { content, .. } => {
                ...
                // 精确识别 compact 摘要：<system-reminder> 标签 + CONTINUATION_HINT 续接指令。
                ...
                // CONTINUATION_HINT 与 nexum-agent::agent::compact::CONTINUATION_HINT 共享——
                // /compact 命令路径（nexum-acp invariant.rs）与 auto-compact 路径
                // （nexum-middlewares compact_middleware.rs）都注入此标记。
                let hint = nexum_agent::agent::compact::CONTINUATION_HINT;
                ...
                let mut blocks: Vec<ContentBlockView> = content
                    ...
                    .map(|block| match block {
                        ContentBlock::Text { text } => {
                            // Redacción de secretos ANTES de renderizar (spec
                            // REABRIR CHAT UX §4): un settings.json con keys
                            // reales impreso por el modelo no debe llegar a
                            // pantalla ni al clipboard de copy.
                            let text = crate::ui::secret_redact::redact_secrets(&text);
            ...
            BaseMessage::Tool {
                ...
                // Redacción de secretos: el caso real del bug (auditoría que
                // leyó .peri/settings.json e imprimió API keys) pasa por
                // acá — el output de Read/Bash/Grep es texto crudo del disco.
                let raw_content =
                    crate::ui::secret_redact::redact_secrets(&content.text_content());
