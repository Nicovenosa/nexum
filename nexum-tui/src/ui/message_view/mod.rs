...
pub use aggregate::{
    aggregate_batch_groups, aggregate_tail_tool_groups, aggregate_tool_groups, group_turn_cards,
};
...
pub enum MessageViewModel {
    ...
    AssistantBubble {
        ...
        /// 预计算的语义 hash（构造/变更时更新，rebuild 直接读取避免重算）
        content_hash: u64,
        /// 字符数（footer metadata，不参与 hash/eq）
        char_count: usize,
        /// 响应耗时 ms（footer metadata，不参与 hash/eq）
        elapsed_ms: u64,
        /// token 数（footer metadata，不参与 hash/eq）
        token_count: usize,
        /// copied 标签显示截止时间（footer metadata，不参与 hash/eq）
        copied_label_until: Option<std::time::Instant>,
        /// Turn grouping (sprint turn-card 2026-07-07): mostrar el header
        /// "◇ NEXUM". Solo la PRIMERA burbuja de un turno lo muestra; las
        /// siguientes son continuación (borde │ sin header) → un solo NEXUM
        /// por turno. SÍ participa del hash (invalida cache al cambiar).
        show_header: bool,
        /// Mostrar el footer (separador + metadata). Solo la ÚLTIMA burbuja de
        /// un turno lo muestra → un solo footer/metadata por turno. SÍ hash.
        show_footer: bool,
    },
    /// 工具调用结果
    ToolBlock {
    ...
    fn hash<H: Hasher>(&self, state: &mut H) {
        match self {
            ...
            MessageViewModel::AssistantBubble {
                ...
                show_header,
                show_footer,
                // UI-only footer metadata se excluye del hash
                ..
            } => {
                1u8.hash(state);
                blocks.hash(state);
                ...
                // Turn grouping: SÍ participan del hash para invalidar cache
                // cuando una burbuja pasa de última→no-última (pierde footer).
                show_header.hash(state);
                show_footer.hash(state);
            }
            MessageViewModel::ToolBlock {
                tool_name,
                tool_call_id,
...
impl MessageViewModel {
    ...
    /// 提取 AssistantBubble 的原始文本内容（用于 copy response）。
    /// 仅返回 Text block 的 raw 内容，跳过 Reasoning 和 ToolUse。
    pub fn assistant_raw_text(&self) -> Option<String> {
        match self {
            MessageViewModel::AssistantBubble { blocks, .. } => {
                let text: String = blocks
                    .iter()
                    .filter_map(|b| match b {
                        ContentBlockView::Text { raw, .. } if !raw.trim().is_empty() => {
                            Some(raw.as_str())
                        }
                        _ => None,
                    })
