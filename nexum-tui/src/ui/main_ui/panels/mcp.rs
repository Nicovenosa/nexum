use nexum_middlewares::mcp::{ClientStatus, ConfigSource, OAuthStatus, ServerInfo};
use nexum_widgets::{BorderedPanel, ScrollState, ScrollableArea};
use ratatui::{
    layout::Rect,
...
fn render_server_group(
    lines: &mut Vec<Line>,
    servers: &[&nexum_middlewares::mcp::ServerInfo],
    global_cursor: usize,
    start_offset: &usize,
    lc: &LcRegistry,
) {
...
fn partition_by_source(
    ...
    Vec<&nexum_middlewares::mcp::ServerInfo>,
    Vec<&nexum_middlewares::mcp::ServerInfo>,
) {
...
#[cfg(test)]
mod tests {
    use nexum_middlewares::mcp::{ClientStatus, ConfigSource, ServerInfo};
