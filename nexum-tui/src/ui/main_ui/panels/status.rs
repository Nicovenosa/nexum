...
fn estimate_cost(app: &App) -> f64 {
    let tracker = &app.session_mgr.current().agent.session_token_tracker;
    let alias_owned = app.services.nexum_config.read().config.active_alias.clone();
...
fn build_bar_chart_lines(
    history: &[nexum_agent::agent::token::RequestRecord],
    chart_width: usize,
    chart_height: usize,
) -> Vec<Line<'static>> {
...
fn build_cache_rate_lines(
    ...
    let rates: Vec<u64> = visible
        .iter()
        // Los registros sin muestras no entran al gráfico: un punto en 0% que
        // en realidad es "no hubo llamadas" deforma la escala y miente.
        .filter_map(|r| r.cache_hit_rate())
        .map(|rate| (rate * 100.0) as u64)
        .collect();
    ...
    for row in (0..chart_height).rev() {
        ...
        for record in visible {
            let Some(rate) = record.cache_hit_rate().map(|r| (r * 100.0) as u64) else {
                continue;
            };
            if rate < row_bottom {
                spans.push(Span::raw(" "));
...
#[cfg(test)]
mod tests {
    use nexum_agent::agent::token::RequestRecord;
