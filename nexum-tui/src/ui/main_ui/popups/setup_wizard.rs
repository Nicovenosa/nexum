...
use super::wizard_chrome as chrome;
use crate::{
    app::setup_wizard::{
...
pub(crate) fn render_setup_wizard(f: &mut Frame, app: &mut crate::app::App) {
    ...
    match step {
        SetupStep::Choose => {
            ...
            render_step_choose(f, wizard, lc, area, tick);
        }
        SetupStep::Language => {
            let lc = &app.services.lc;
            let wizard = app.global_ui.setup_wizard.as_ref().unwrap();
            render_step_language(f, wizard, lc, area, tick);
        }
        SetupStep::Form => render_step_form(f, app, area),
        SetupStep::Done => {
            let lc = &app.services.lc;
            let wizard = app.global_ui.setup_wizard.as_ref().unwrap();
            render_step_done(f, wizard, lc, area, tick);
        }
    }
}
...
fn render_step_choose(
    ...
    let cuerpo = chrome::render_encabezado(f, area, tick);
    let mut lines = chrome::lineas_titulo(
        &lc.tr("setup-welcome-title"),
        &lc.tr("setup-choose-provider"),
    );

    for (i, src) in SetupSource::ALL.iter().enumerate() {
        let sel = i == wizard.choose_cursor;
        lines.push(chrome::linea_opcion(sel, &src.label(lc)));
        lines.push(chrome::linea_detalle(sel, &src.description(lc)));
        lines.push(Line::from(""));
    }

    f.render_widget(
        Paragraph::new(Text::from(lines)),
        chrome::zona_cuerpo(cuerpo),
    );
    chrome::render_footer(
        f,
        cuerpo,
        &[
            ("↑↓", lc.tr("setup-key-select")),
            ("Enter", lc.tr("setup-key-confirm")),
            ("Esc", lc.tr("setup-key-quit")),
        ],
    );
}
...
fn render_step_language(
    ...
    let cuerpo = chrome::render_encabezado(f, area, tick);
    // El idioma se elige PRIMERO y todo lo demás lo sigue — incluida esta
    // pantalla, que se redibuja en el idioma nuevo apenas se confirma.
    let mut lines = chrome::lineas_titulo(
    ...
    for (i, (_code, name)) in LANGUAGE_OPTIONS.iter().enumerate() {
        lines.push(chrome::linea_opcion(i == wizard.language_cursor, name));
    }
...
fn render_step_form(f: &mut Frame, app: &mut crate::app::App, area: Rect) {
    ...
    match wizard.form_mode {
        FormMode::Browse => render_form_browse(f, wizard, lc, area, tick),
        FormMode::Edit => render_form_edit(f, wizard, lc, area, tick),
    }
}
...
fn render_form_browse(
    ...
    let cuerpo = chrome::render_encabezado(f, area, tick);
    let inner = chrome::zona_cuerpo(cuerpo);
    let mut lines: Vec<Line> = chrome::lineas_titulo(&lc.tr("setup-configure-title"), "");
    ...
    for (idx, mp) in wizard.providers.iter().enumerate() {
        ...
        let check_char = if mp.selected { "x" } else { " " };
    ...
    chrome::render_footer(
        ...
        &[
            ("↑↓", lc.tr("setup-key-select")),
            ("Space", lc.tr("setup-key-check")),
            ("Enter", lc.tr("setup-key-edit-submit")),
            ("Esc", lc.tr("setup-key-back")),
        ],
    );
}
...
fn render_form_edit(
    ...
    let cuerpo = chrome::render_encabezado(f, area, tick);
    let inner = chrome::zona_cuerpo(cuerpo);
    let mut lines: Vec<Line> = chrome::lineas_titulo(&header, "");
...
fn render_step_done(
    ...
    let cuerpo = chrome::render_encabezado(f, area, tick);
    let inner = chrome::zona_cuerpo(cuerpo);
    let mut lines = chrome::lineas_titulo(&lc.tr("setup-complete-title"), "");
    ...
    chrome::render_footer(
        ...
        &[(
            ...
            format!(
                "{}{}",
                lc.tr("setup-press-enter").trim(),
                lc.tr("setup-to-start")
            )
            .trim()
            .to_string(),
        )],
