use nexum_middlewares::prelude::TodoStatus;
use ratatui::{
    layout::Rect,
...
use crate::{
    app::{App, CopyButtonHitbox},
    ui::{
        render_thread::{CachedCopyButton, RenderEvent, WrappedLineInfo},
        theme, welcome,
    },
};
...
pub(crate) fn render_messages(
    ...
    // Los hitboxes del frame anterior expiran SIEMPRE al inicio del render
    // (también en el path welcome) — un click nunca debe caer en un botón
    // que ya no está en pantalla.
    app.session_mgr
        .current_mut()
        .ui
        .copy_button_hitboxes
        .clear();
    ...
    let spinner_line: Option<Line<'static>> = if app.session_mgr.current().ui.loading {
        ...
        let frame = nexum_widgets::spinner::animation::tick_to_frame(session.spinner_state.tick());
        let verb = session.spinner_state.verb();
        let elapsed =
            nexum_widgets::spinner::animation::format_elapsed(session.spinner_state.elapsed_ms());
        ...
        if tokens > 0 {
            let tokens_fmt = nexum_widgets::spinner::animation::format_tokens(tokens);
        ...
        > 0
    {
        let elapsed = nexum_widgets::spinner::animation::format_elapsed(
            app.session_mgr
                .current()
                .spinner_state
                .last_summary_elapsed_ms(),
    ...
    // Debug de render del transcript (NEXUM_RENDER_DEBUG=1). Sin contenido.
    if crate::ui::composer::render_debug_enabled() {
        tracing::info!(
            transcript_rect = ?inner,
            header_rect = ?header_area,
            active_prompt_bar = sticky_header::active_prompt_bar_enabled(),
            scroll_follow = app.session_mgr.current().ui.scroll_follow,
            scroll_offset = app.session_mgr.current().ui.scroll_offset,
            max_scroll,
            "NEXUM_RENDER_DEBUG: transcript render"
        );
    }

    // Active prompt bar (sticky header): sólo si el usuario la habilitó Y hay
    // scroll. Default off — el prompt vive como UserBubble normal, sin barra
    // fija duplicada arriba (la propia render_sticky_header revalida el flag).
    if max_scroll > 0 {
    ...
    // Registrar hitboxes de los botones "📋 Copiar" visibles en
    // este frame (coordenadas de pantalla, ya con scroll offset aplicado).
    {
        let hitboxes = {
            let cache = app.session_mgr.current().messages.render_cache.read();
            compute_copy_button_hitboxes(
                &cache.copy_buttons,
                &cache.wrap_map,
                offset,
                visible_height,
        ...
        app.session_mgr.current_mut().ui.copy_button_hitboxes = hitboxes;
    }
    ...
    if max_scroll > 0 {
        ...
        nexum_widgets::render_vertical_scrollbar(
            f,
            bar_area,
            offset,
            max_scroll,
...
/// Convierte los botones "📋 Copiar" del RenderCache a hitboxes de
/// pantalla para el frame actual. Función pura (testeable sin App):
///
/// - Aplica el scroll `offset`: un botón cuya fila visual queda fuera de
///   `[offset, offset + visible_height)` NO se registra (no es clickeable).
/// - `wrap_map[line_idx].visual_row_start` ubica la primera fila visual de
///   la línea del botón (la línea es corta; si el terminal es más angosto
///   que el label, el hitbox cubre solo la primera fila — degradación
///   aceptable).
/// - El ancho se clampea al borde derecho de `text_area`.
pub(crate) fn compute_copy_button_hitboxes(
    copy_buttons: &[CachedCopyButton],
    wrap_map: &[WrappedLineInfo],
    offset: u16,
    visible_height: u16,
    text_area: Rect,
) -> Vec<CopyButtonHitbox> {
    let mut hitboxes = Vec::new();
    for btn in copy_buttons {
        let Some(info) = wrap_map.get(btn.line_idx) else {
            continue;
        };
        let vrow = info.visual_row_start;
        if vrow < offset || vrow >= offset.saturating_add(visible_height) {
            continue;
        }
        let y = text_area.y + (vrow - offset);
        let x = text_area.x.saturating_add(btn.col_start);
        if x >= text_area.right() {
            continue;
        }
        let width = btn.width.min(text_area.right() - x);
        if width == 0 {
            continue;
        }
        hitboxes.push(CopyButtonHitbox {
            rect: Rect {
                x,
                y,
                width,
