...
use super::{
    ...
    message_render::{render_view_model_with_copy_button, CopyButtonRel},
    message_view::MessageViewModel,
};

/// Botón "📋 Copiar" ya resuelto a coordenadas del cache:
/// `line_idx` indexa `RenderCache.lines` (espacio deduplicado);
/// `message_idx` indexa `view_messages` (mismo espacio que el snapshot
/// recibido en Rebuild). El UI thread lo convierte a Rect de pantalla
/// aplicando wrap_map + scroll offset (ver message_area.rs).
#[derive(Debug, Clone, Copy)]
pub struct CachedCopyButton {
    pub line_idx: usize,
...
pub struct RenderCache {
    ...
    /// Botones "Copiar respuesta" presentes en `lines` (uno por turno
    /// completado, ver render_view_model_with_copy_button).
    pub copy_buttons: Vec<CachedCopyButton>,
}
...
pub enum RenderEvent {
    ...
    ToggleDiff(bool),
    /// 更新 i18n locale para labels del footer (Ctrl+C copy / copiar)
    SetLocale(String),
    /// 更新 label resuelto del botón "📋 Copiar" desde LcRegistry.
    /// Usado para unificar el resolver con la statusbar.
    SetCopyButtonLabel(String),
    /// 高亮 copy-button hover：message_idx del botón bajo el cursor (None = sin hover).
    SetCopyButtonHover(Option<usize>),
}
...
struct RenderTask {
    ...
    /// Posición relativa del botón "📋 Copiar" por mensaje
    /// (paralelo a message_lines, se reusa/invalida junto con él)
    message_buttons: Vec<Option<CopyButtonRel>>,
    ...
    diff_visible: bool,
    locale: String,
    /// Label resuelto del botón "📋 Copiar" (desde LcRegistry).
    copy_button_label: String,
    /// Message index del botón "📋 Copiar" bajo el cursor (hover). None = sin hover.
    copy_button_hover: Option<usize>,
}
...
impl RenderTask {
    ...
    fn render_one(
        ...
        let (mut lines, button_rel) = render_view_model_with_copy_button(
            vm,
            Some(index),
            width,
            diff_visible,
    ...
    fn is_cosmetic_change(old: &MessageViewModel, new: &MessageViewModel) -> bool {
        match (old, new) {
            // AssistantBubble: blocks 内容相同，仅 flags 变化。
            // OJO: el flip is_streaming true→false en la última burbuja del
            // turno (show_footer) NO es cosmético — al terminar el streaming
            // aparece el botón "📋 Copiar", que requiere re-render.
            // Costo: un render extra de UN mensaje por turno completado.
            (
                MessageViewModel::AssistantBubble {
                    blocks: old_blocks,
                    collapsed: old_collapsed,
                    is_streaming: old_streaming,
                    show_footer: old_footer,
                    ..
                },
                MessageViewModel::AssistantBubble {
                    blocks: new_blocks,
                ...
                old_blocks == new_blocks
                    ...
                    && (old_streaming == new_streaming || !*new_footer)
            }
    ...
    fn rebuild(&mut self, messages: Vec<MessageViewModel>) {
        ...
        let copy_labels = super::message_render::copy_labels(&self.locale);
        let button_label = self.copy_button_label.as_str();
        ...
        let old_message_buttons = std::mem::take(&mut self.message_buttons);

        // 调整 message_lines 容量
        self.message_lines.resize(new_len, Vec::new());
        self.message_buttons.resize(new_len, None);

        // 复用前缀的缓存行
        for i in 0..prefix_stable_len {
            if i < old_message_lines.len() {
                self.message_lines[i] = std::mem::take(&mut old_message_lines[i]);
                self.message_buttons[i] = old_message_buttons.get(i).copied().flatten();
            }
        }
        ...
        for (i, mut vm) in messages.into_iter().enumerate() {
            ...
            let (lines, button_rel) = Self::render_one(
                &mut vm,
                i + 1,
                width,
                self.diff_visible,
                ...
                &copy_labels.1,
                button_label,
                self.copy_button_hover,
            );
            self.message_lines[i] = lines;
            self.message_buttons[i] = button_rel;
        }
        ...
        let mut copy_buttons: Vec<CachedCopyButton> = Vec::new();
        let mut prev_empty = false;
        for (msg_idx, lines) in self.message_lines.iter().enumerate() {
            offsets.push(deduped.len());
            let button_rel = self.message_buttons.get(msg_idx).copied().flatten();
            for (line_offset, line) in lines.iter().enumerate() {
                ...
                // La línea del botón nunca es vacía → siempre sobrevive al
                // dedup; acá se resuelve su índice en el espacio deduplicado.
                if let Some(rel) = button_rel {
                    if rel.line_offset == line_offset {
                        copy_buttons.push(CachedCopyButton {
                            line_idx: deduped.len() - 1,
                            message_idx: msg_idx,
                            col_start: rel.col_start,
                            width: rel.width,
                        });
                    }
                }
        ...
        cache.copy_buttons = copy_buttons;
        cache.version += 1;
    }
        ...
        while let Some(event) = rx.recv().await {
            match event {
                ...
                RenderEvent::Clear => {
                    ...
                    self.message_buttons.shrink_to_fit();
                    self.message_hashes.clear();
                ...
                RenderEvent::SetLocale(locale) => {
                    self.locale = locale;
                    // Labels del footer cambiaron → forzar rebuild completo
                    self.message_hashes.clear();
                    if !self.last_messages.is_empty() {
                        let messages = std::mem::take(&mut self.last_messages);
                        self.rebuild_safe(messages);
                ...
                RenderEvent::SetCopyButtonLabel(label) => {
                    self.copy_button_label = label;
                    // Label del botón cambió → forzar rebuild completo
                    self.message_hashes.clear();
                    if !self.last_messages.is_empty() {
                        let messages = std::mem::take(&mut self.last_messages);
                        self.rebuild_safe(messages);
                ...
                RenderEvent::SetCopyButtonHover(hover) => {
                    if self.copy_button_hover != hover {
                        self.copy_button_hover = hover;
                        // Hover cambia el color del botón → forzar rebuild completo
                        self.message_hashes.clear();
                        if !self.last_messages.is_empty() {
                            let messages = std::mem::take(&mut self.last_messages);
                            self.rebuild_safe(messages);
...
pub fn spawn_render_thread(
    ...
    let task = RenderTask {
        ...
        diff_visible: false,
        locale: "en".to_string(),
        copy_button_label: super::message_render::copy_button_label("en").to_string(),
        copy_button_hover: None,
    };
