...
async fn test_welcome_card_renders_when_empty() {
    ...
    assert!(
        snap_text.contains("Nexum"),
        "Welcome Card 应包含 'Nexum'，实际:\n{}",
        snap_text
    );
    assert!(
...
async fn test_welcome_card_shows_login_guide_when_no_provider() {
    // 无 Provider 时 Welcome Card 应显示首次引导
    let (mut app, mut handle) = App::new_headless(120, 30).await;
    // Simular sin provider
    app.services.provider_name = String::new();
    handle
        .terminal
        .draw(|f| main_ui::render(f, &mut app))
        .unwrap();
    ...
    let snap_text = snap.join("\n");
    assert!(
        snap_text.contains("provider"),
        "无 Provider 时 Welcome Card 应显示 provider 引导，实际:\n{}",
        snap_text
    );
}
...
/// Rellena la sesión con suficientes mensajes para forzar scroll y setea
/// un last_human_message que NO está en el transcript (así, si aparece en el
/// snapshot, es exclusivamente por la sticky bar).
async fn setup_scrolling_session_with_sticky_probe(
    app: &mut App,
    handle: &mut crate::ui::headless::HeadlessHandle,
) {
    for i in 0..30 {
        let notified = handle.render_notify.notified();
    ...
    app.session_mgr.current_mut().metadata.last_human_message =
        Some("STICKYPROBE hello from user".to_string());
}
...
async fn test_active_prompt_bar_off_por_default_no_sticky() {
    ...
    std::env::remove_var("NEXUM_ACTIVE_PROMPT_BAR");

    let (mut app, mut handle) = App::new_headless(80, 24).await;
    setup_scrolling_session_with_sticky_probe(&mut app, &mut handle).await;
    handle
        .terminal
        .draw(|f| main_ui::render(f, &mut app))
        .unwrap();
    let snap_text = handle.snapshot().join("\n");

    assert!(
        !snap_text.contains("STICKYPROBE"),
        "con la active prompt bar OFF (default), el prompt NO debe aparecer \
         pegado arriba; snapshot:\n{}",
        snap_text
    );

    match prev {
        Some(v) => std::env::set_var("NEXUM_ACTIVE_PROMPT_BAR", v),
        None => std::env::remove_var("NEXUM_ACTIVE_PROMPT_BAR"),
    }
}
...
async fn test_active_prompt_bar_on_muestra_sticky() {
    let _g = PROVIDER_ENV_TEST_LOCK.lock().unwrap();
    let prev = std::env::var("NEXUM_ACTIVE_PROMPT_BAR").ok();
    std::env::set_var("NEXUM_ACTIVE_PROMPT_BAR", "on");
    ...
    assert!(
        snap_text.contains("STICKYPROBE"),
        "con NEXUM_ACTIVE_PROMPT_BAR=on el prompt sticky debe aparecer; \
         snapshot:\n{}",
        snap_text
    );
...
async fn test_cron_panel_render() {
    use chrono::Utc;
    use nexum_middlewares::cron::CronTask;

    let (mut app, mut handle) = App::new_headless(120, 30).await;

    // Host-provided cron snapshot, not a local scheduler.
    app.services.cron.set_visible_tasks(vec![CronTask {
        id: "host-job-1".to_string(),
        expression: "* * * * *".to_string(),
        prompt: "hello cron test".to_string(),
...
async fn test_tab_bar_integration() {
    ...
    use nexum_middlewares::ask_user::{AskUserBatchRequest, AskUserOption, AskUserQuestionData};
...
async fn test_status_bar_shows_permission_mode() {
    ...
    // UX FIX 05.6: "Bypass" ya no se muestra. La aprobación visual por defecto es Manual.
    handle
        .terminal
        .draw(|f| crate::ui::main_ui::render(f, &mut app))
        .unwrap();
    ...
    assert!(
        snap.contains("Aprobación Manual"),
        "状态栏应 mostrar aprobación Manual por defecto, actual:\n{}",
        snap
    );
    assert!(
        !snap.contains("Bypass"),
        "状态栏 no debe mostrar Bypass, actual:\n{}",
        snap
    );
}

/// Sprint A (Bug 5): la statusbar ya no lee services.provider_name (copia
/// stale) sino runtime_identity(&NexumConfig). Los tests setean la config.
fn set_active_provider_for_test(app: &App, _name: &str, model: &str) {
    let (provider_id, base_url, display_name) = if model.starts_with("qwen") {
        ("ollama_local", "http://127.0.0.1:11434/v1", "Ollama Local")
    } else if model.starts_with("gpt-") {
        ("codex_cli", "http://127.0.0.1:8317/v1", "Codex / OpenAI")
    } else {
        ("opencode_go", "https://opencode.ai/go/v1", "OpenCode Go")
    };
    let mut cfg = app.services.nexum_config.write();
    cfg.config.active_provider_id = provider_id.to_string();
    cfg.config.active_alias = model.to_string();
...
#[tokio::test]
async fn test_status_bar_shows_nexum_provider_and_real_model_without_cwd() {
    let (mut app, mut handle) = App::new_headless(120, 24).await;
    set_active_provider_for_test(&app, "OpenAI", "deepseek-v4-flash");
    app.services.cwd = "nexum-agent-app-cli".to_string();
    let _env_guard = PROVIDER_ENV_TEST_LOCK.lock().unwrap();
    // Evitar pollution de env var de tests paralelos
    std::env::remove_var("NEXUM_PROVIDER_PROFILE");
    std::env::remove_var("NEXUM_PROVIDER");
    ...
    assert!(
        snap.contains(
            "Nexum · Aprobación Manual · Modo Build · OpenCode Go · deepseek-v4-flash · MEM"
        ),
        "状态栏应 mostrar marca, aprobación, modo, provider, modelo y MEM, actual:\n{}",
        snap
    );
    assert!(
        !snap.contains("Bypass"),
    ...
    assert!(
        !snap.contains("OpenCode Go · nexum-agent-app-cli · deepseek-v4-flash"),
        "状态栏不应在 Provider 和模型之间显示 cwd，实际:\n{}",
        snap
    );
}

#[tokio::test]
async fn test_status_bar_shows_ollama_local_provider_label() {
    let (mut app, mut handle) = App::new_headless(120, 24).await;
    set_active_provider_for_test(&app, "OpenAI", "qwen2.5:0.5b");
    handle
        .terminal
        .draw(|f| crate::ui::main_ui::render(f, &mut app))
        .unwrap();
    ...
    assert!(
        snap.contains("Nexum · Aprobación Manual · Modo Build · Ollama Local · qwen2.5:0.5b · MEM"),
        "状态栏应 mostrar Ollama Local con aprobación/modo primero, actual:\n{}",
        snap
    );
    assert!(
        !snap.contains("Bypass"),
    ...
    assert!(
        !snap.contains("Nexum · Aprobación Manual · Modo Build · OpenAI · qwen2.5:0.5b"),
        "Ollama profile should not be displayed as OpenAI，实际:\n{}",
        snap
    );
}

#[tokio::test]
async fn test_status_bar_ux_05_5_shortcuts_are_left_aligned_and_clean() {
    let (mut app, mut handle) = App::new_headless(120, 24).await;
    set_active_provider_for_test(&app, "OpenAI", "qwen2.5:0.5b");
    handle
        .terminal
    ...
    let snap = rows.join("\n");
    let shortcut_row = rows
        .iter()
        .find(|row| row.contains("/ :Comando") || row.contains("/ Cmd"))
        .expect("footer debe renderizar una fila de atajos");

    assert!(
        snap.contains("Nexum · Aprobación Manual · Modo Build · Ollama Local · qwen2.5:0.5b · MEM"),
        "footer debe mantener provider/model reales con aprobación/modo primero, actual:\n{}",
        snap
    );
    assert!(
        !snap.contains("Bypass"),
        "footer no debe mostrar Bypass, actual:\n{}",
        snap
    );
    assert!(
        shortcut_row.starts_with("/ :Comando") || shortcut_row.starts_with("/ Cmd"),
        "la fila de atajos debe arrancar alineada a la izquierda, actual: {:?}",
        shortcut_row
    );
    assert!(
        snap.contains("Tab :Modo"),
        "footer debe anunciar Tab :Modo, actual:\n{}",
        snap
    );
    assert!(
        !snap.contains("Ctrl+M"),
        "footer no debe anunciar Ctrl+M, actual:\n{}",
        snap
    );
    assert!(
        !snap.contains("Ctrl+U/D") && !snap.contains("Subir/Bajar") && !snap.contains("Scroll"),
        "footer no debe anunciar scroll shortcuts, actual:\n{}",
        snap
    );
}

#[tokio::test]
async fn test_status_bar_ux_05_6_footer_priority_and_no_bypass() {
    let (mut app, mut handle) = App::new_headless(120, 24).await;
    set_active_provider_for_test(&app, "OpenAI", "qwen2.5:0.5b");
    app.global_ui.agent_mode = crate::app::AgentMode::Research;
    app.global_ui.approval_display_mode = crate::app::ApprovalDisplayMode::Auto;
    handle
        .terminal
        .draw(|f| crate::ui::main_ui::render(f, &mut app))
        .unwrap();
    ...
    let status_row = rows
        .iter()
        .find(|row| row.starts_with(" Nexum"))
        .expect("footer debe renderizar la fila de estado inicial");
    ...
    assert!(
        status_row.contains("Aprobación Auto") || status_row.contains("Auto"),
        "footer debe mostrar aprobación cerca del inicio, actual: {:?}",
        status_row
    );
    assert!(
        status_row.contains("Modo Research") || status_row.contains("Research"),
        "footer debe mostrar modo cerca del inicio, actual: {:?}",
        status_row
    );
    let auto_count = status_row.matches("Auto").count();
    assert_eq!(
        auto_count, 1,
        "footer no debe duplicar aprobación, actual: {:?}",
        status_row
    );
    assert!(
        status_row.contains("Ollama Local"),
        "footer debe mantener provider, actual: {:?}",
        status_row
    );
    assert!(
        status_row.contains("qwen2.5:0.5b"),
        "footer debe mantener modelo, actual: {:?}",
        status_row
    );
    assert!(
        status_row.contains("MEM"),
        "footer debe mantener MEM, actual: {:?}",
        status_row
    );
    let approval_pos = status_row.find("Auto").unwrap();
    let provider_pos = status_row.find("Ollama Local").unwrap();
    assert!(
        approval_pos < provider_pos,
        "aprobación debe aparecer antes que provider, actual: {:?}",
        status_row
    );
}

#[tokio::test]
async fn test_mascot_not_rendered_in_header_after_chat_by_default() {
    let (mut app, mut handle) = App::new_headless(120, 24).await;
    app.session_mgr
        .current_mut()
        .messages
        .view_messages
        .push(MessageViewModel::user("hola".to_string()));
    ...
    assert!(
        ...
        "La mascota no debe flotar en el header durante chat activo, actual:\n{}",
        snap
    );
    assert!(
        snap.contains("NEXUM"),
        "El header compacto debe mantener identidad Nexum sin mascota flotante, actual:\n{}",
        snap
    );
}
...
async fn test_status_bar_updates_after_mode_switch() {
    let (mut app, mut handle) = App::new_headless(120, 24).await;
    // UX FIX 05.6: la aprobación del footer ahora refleja approval_display_mode.
    // Manual por defecto
    app.global_ui.approval_display_mode = crate::app::ApprovalDisplayMode::Manual;
    handle
        .terminal
        .draw(|f| crate::ui::main_ui::render(f, &mut app))
        .unwrap();
    assert!(
        ...
        "no debe filtrar texto interno DEFAULT, actual:\n{}",
        handle.snapshot().join("\n")
    );
    assert!(
        handle.contains("Aprobación Manual"),
        "footer debe mostrar Aprobación Manual, actual:\n{}",
        handle.snapshot().join("\n")
    );

    // Cambiar a Parcial
    app.global_ui.approval_display_mode = crate::app::ApprovalDisplayMode::Partial;
    handle
        .terminal
        .draw(|f| crate::ui::main_ui::render(f, &mut app))
        .unwrap();
    assert!(
        handle.contains("Aprobación Parcial"),
        "footer debe mostrar Aprobación Parcial, actual:\n{}",
        handle.snapshot().join("\n")
    );
    ...
    handle
        ...
        .unwrap();
    assert!(
        handle.contains("Aprobación Auto"),
        "footer debe mostrar Aprobación Auto, actual:\n{}",
        handle.snapshot().join("\n")
    );
}
...
async fn test_tool_call_widget_renders_completed() {
    ...
    let (lines, _) = crate::ui::message_render::render_view_model(
        &vm,
        Some(1),
        80,
        false,
...
fn make_compact_done_event(summary: &str, re_inject_parts: &[&str]) -> AgentEvent {
    ...
    for part in re_inject_parts {
        if let Some(rest) = part.strip_prefix("[最近读取的文件: ") {
            ...
            if !path.is_empty() {
                files.push(nexum_agent::agent::events::CompactFileInfo {
                    path: path.to_string(),
                    lines: line_count,
                });
            }
...
async fn test_get_compact_config_default() {
    ...
    let default = nexum_agent::agent::CompactConfig::default();
...
async fn test_get_compact_config_from_settings() {
    let (mut app, _handle) = App::new_headless(120, 30).await;
    let mut zen = crate::config::NexumConfig::default();
    zen.config.compact = Some(nexum_agent::agent::CompactConfig {
        auto_compact_threshold: 0.9,
        ..Default::default()
    });
    app.services.nexum_config = std::sync::Arc::new(parking_lot::RwLock::new(zen));
    let config = app.get_compact_config();
    assert!(
...
async fn test_messages_accumulate_across_turns() {
    ...
    // nexum-agent/src/agent/executor/final_answer.rs::emit_snapshot_and_drain_notifications
...
fn test_state_snapshot_is_incremental() {
    use nexum_agent::messages::{BaseMessage, MessageContent, MessageId};
...
async fn test_unified_hint_shows_commands_and_skills() {
    use nexum_middlewares::skills::loader::SkillMetadata;
...
/// Cron 面板只展示 host 提供的快照，不维护本地 scheduler 状态。
#[tokio::test]
async fn test_cron_panel_is_read_only() {
    use chrono::Utc;
    use nexum_middlewares::cron::CronTask;
    ...
    assert_eq!(
        app.global_panels.get::<CronPanel>().unwrap().tasks().len(),
        1,
        "visible cron state must not be removed locally"
    );
}
...
async fn test_model_panel_confirm_shows_feedback() {
    ...
    app.services.nexum_config = std::sync::Arc::new(parking_lot::RwLock::new(cfg));
    app.session_mgr.current_mut().session_panels.open(
        crate::app::panel_manager::PanelState::Model(ModelPanel::from_config(
            &app.services.nexum_config.read(),
        )),
    );
    app.session_mgr
        .current_mut()
...
async fn test_login_select_provider_shows_feedback() {
    use crate::{
        ...
        config::{AppConfig, NexumConfig, ProviderConfig},
    };
...
async fn test_welcome_shows_model_info() {
    let (mut app, mut handle) = App::new_headless(120, 30).await;
    set_active_provider_for_test(&app, "Codex / OpenAI", "gpt-5.6-terra");
    handle
        .terminal
        .draw(|f| main_ui::render(f, &mut app))
        .unwrap();
...
async fn test_background_task_notification() {
    ...
    app.push_agent_event(AgentEvent::StateSnapshot(vec![
        nexum_agent::messages::BaseMessage::human("test query"),
    ]));
    app.push_agent_event(AgentEvent::Done);
    app.process_pending_events();
...
async fn test_background_task_status_bar() {
    ...
    let notified = handle.render_notify.notified();
    app.push_agent_event(AgentEvent::StateSnapshot(vec![
        nexum_agent::messages::BaseMessage::human("test"),
    ]));
    app.push_agent_event(AgentEvent::Done);
    app.process_pending_events();
    notified.await;
...
async fn test_subagent_group_preserved_after_done_reconcile() {
    use nexum_agent::messages::{BaseMessage, MessageContent, ToolCallRequest};
...
async fn test_diagnostic_bg_subagent_group_disappears() {
    use nexum_agent::messages::{BaseMessage, ToolCallRequest};
    ...
    // 验证 bg_task_state.agent_done_pending 被设置
    assert!(
        app.session_mgr.current_mut().agent.bg_task_state.agent_done_pending,
        "Done with !background_agents.is_empty() should set bg_task_state.agent_done_pending = true"
    );
...
///
/// 这导致：
/// 1. Done 时 !background_agents.is_empty() → bg_task_state.agent_done_pending = true
...
async fn test_diagnostic_fork_plus_background_subagent_group() {
    ...
    // Step 5: Done — 此时 background_agents.len()=1，触发 bg_task_state.agent_done_pending
    app.push_agent_event(AgentEvent::Done);
    app.process_pending_events();
    bg_diag_print_vms(&app, "Fork+BG Step 4: After Done");

    // 验证 bg_task_state.agent_done_pending 被设置（因为 !background_agents.is_empty()）
    assert!(
        app.session_mgr.current_mut().agent.bg_task_state.agent_done_pending,
        "Done with !background_agents.is_empty() should set bg_task_state.agent_done_pending = true"
    );
...
async fn test_thinking_toolcall_text_rebuild_preserves_user() {
    use nexum_agent::messages::{BaseMessage, ContentBlock, MessageContent, MessageId};
...
// ─── Fix chat 2026-07-06: scrollback + live-follow + input aislado ───────────

#[tokio::test]
async fn test_scroll_up_apaga_live_follow_y_bottom_lo_reactiva() {
    let (mut app, _handle) = App::new_headless(80, 24).await;
    assert!(
        app.session_mgr.current().ui.scroll_follow,
        "estado inicial: live-follow ON"
    );
    app.scroll_up();
    assert!(
        !app.session_mgr.current().ui.scroll_follow,
        "scrollear arriba debe apagar live-follow"
    );
    app.scroll_to_bottom();
    assert!(
        app.session_mgr.current().ui.scroll_follow,
        "volver al final (End) debe reactivar live-follow"
    );
}

#[tokio::test]
async fn test_scroll_transcript_no_toca_el_input_buffer() {
    let (mut app, _handle) = App::new_headless(80, 24).await;
    app.session_mgr
        .current_mut()
        .ui
        .textarea
        .insert_str("auditoría en progreso");
    ...
    assert_eq!(
        lines,
        vec!["auditoría en progreso".to_string()],
        "el scroll del transcript NUNCA modifica el input buffer"
    );
    // El offset sí se movió (scroll real, no no-op).
    let _ = offset_before;
}

// ─── Sprint ghost-text 2026-07-07: prediction gate + composer limpio ─────────
...
async fn test_prediction_ready_descartada_con_flag_off() {
    ...
    std::env::remove_var("NEXUM_PREDICTIONS"); // default = off

    let (mut app, _handle) = App::new_headless(80, 24).await;
    let notif = crate::acp_client::client::AcpNotification::PredictionReady {
        session_id: "s1".to_string(),
        text: "nexum-agent 的 ReAct loop predicho".to_string(),
    };
    app.handle_acp_notification(notif);
    assert!(
        app.session_mgr.current().ui.prediction.is_none(),
        "con predictions off, la prediction no debe almacenarse (nada de ghost text)"
    );

    match prev {
        Some(v) => std::env::set_var("NEXUM_PREDICTIONS", v),
        None => std::env::remove_var("NEXUM_PREDICTIONS"),
    }
}
...
async fn test_prediction_ready_almacenada_con_flag_on() {
    let _g = PROVIDER_ENV_TEST_LOCK.lock().unwrap();
    let prev = std::env::var("NEXUM_PREDICTIONS").ok();
    std::env::set_var("NEXUM_PREDICTIONS", "on");

    let (mut app, _handle) = App::new_headless(80, 24).await;
    let notif = crate::acp_client::client::AcpNotification::PredictionReady {
        session_id: "s1".to_string(),
        text: "sugerencia".to_string(),
    };
    app.handle_acp_notification(notif);
    assert!(
        app.session_mgr.current().ui.prediction.is_some(),
        "con predictions on y textarea vacío, la prediction se almacena"
    );
...
fn prepare_loading_turn(app: &mut App, prompt: &str) {
    app.session_mgr.current_mut().messages.last_submitted_text = Some(prompt.to_string());
    app.session_mgr.current_mut().agent.turn_terminal = None;
    app.session_mgr.current_mut().agent.prompt_restoration_count = 0;
    app.session_mgr.current_mut().ui.textarea = crate::app::build_textarea(false);
    app.set_loading(true);
}

#[tokio::test]
async fn transport_eof_clears_loading_and_restores_prompt_once() {
    let (mut app, _handle) = App::new_headless(80, 24).await;
    prepare_loading_turn(&mut app, "Hola");
    app.handle_acp_notification(crate::acp_client::client::AcpNotification::TurnFailed {
...
#[tokio::test]
async fn duplicate_terminal_is_ignored_by_tui() {
    let (mut app, _handle) = App::new_headless(80, 24).await;
    prepare_loading_turn(&mut app, "Hola");
    app.handle_acp_notification(crate::acp_client::client::AcpNotification::TurnFailed {
        message: "first".into(),
    });
    app.handle_acp_notification(crate::acp_client::client::AcpNotification::AgentDone {
        session_id: "s".into(),
    });
...
#[tokio::test]
async fn timed_out_terminal_clears_loading_and_restores_prompt_once() {
    let (mut app, _handle) = App::new_headless(80, 24).await;
    prepare_loading_turn(&mut app, "consulta");
    app.handle_acp_notification(crate::acp_client::client::AcpNotification::TurnTerminal {
        state: nexum_acp::session::terminal::TerminalState::TimedOut,
        message: Some("provider silent".into()),
    });
...
#[tokio::test]
async fn envelope_conversion_failure_restores_prompt_once() {
    let mut source = crate::hormiguero::TaskEnvelope {
        envelope_id: "env".into(),
        trace_id: "trace".into(),
        turn_id: "turn".into(),
        request_id: "request".into(),
        route_decision: crate::hormiguero::StableRouteDecision::OneShot,
        task_classification: crate::hormiguero::StableTaskClassification::Simple,
    ...
    let context = crate::hormiguero::acp_adapter::EnvelopeConversionContext {
    ...
    let error =
        crate::hormiguero::acp_adapter::to_acp_task_envelope(&source, &context).unwrap_err();
...
#[tokio::test]
async fn cancellation_restores_prompt_once() {
    let (mut app, _handle) = App::new_headless(80, 24).await;
    prepare_loading_turn(&mut app, "cancelame");
    app.handle_acp_notification(crate::acp_client::client::AcpNotification::TurnTerminal {
        state: nexum_acp::session::terminal::TerminalState::Cancelled,
...
#[tokio::test]
async fn explicit_control_local_answer_does_not_call_acp() {
    let (mut app, _handle) = App::new_headless(80, 24).await;
    assert!(app.acp_client.is_none());
    app.submit_message("pará".into());
    assert!(!app.session_mgr.current().ui.loading);
    assert_eq!(
        app.session_mgr.current().agent.turn_terminal,
...
async fn poll_until_terminal(app: &mut App) {
    for _ in 0..100 {
        tokio::task::yield_now().await;
        app.poll_agent();
    ...
    panic!("turn did not reach terminal state");
}

fn install_test_provider(app: &App) {
    let mut cfg = app.services.nexum_config.write();
    cfg.config.active_provider_id = "stable-test".into();
...
fn install_test_acp(app: &mut App) -> nexum_acp::transport::mpsc::MpscServerTransport {
    use std::sync::Arc;
    let (transport, server) = nexum_acp::transport::mpsc::mpsc_transport_pair();
    let transport: Arc<dyn crate::acp_client::AcpClientTransport> = Arc::new(transport);
    let (client, notifications) = crate::acp_client::AcpTuiClient::new(transport);
    client.spawn_pump();
...
impl nexum_agent::llm::BaseModel for StableE2eModel {
    async fn invoke(
        &self,
        _request: nexum_agent::llm::LlmRequest,
    ) -> nexum_agent::error::AgentResult<nexum_agent::llm::LlmResponse> {
        use std::sync::atomic::Ordering;
        self.calls.fetch_add(1, Ordering::SeqCst);
        Ok(nexum_agent::llm::LlmResponse {
            message: nexum_agent::messages::BaseMessage::ai("respuesta"),
            stop_reason: nexum_agent::llm::StopReason::EndTurn,
            usage: None,
            request_id: None,
        })
    }
...
struct StableSessionEvidence {
    app: App,
    request_count: usize,
    envelopes: Vec<nexum_acp::task::TaskEnvelopeV1>,
    flows: Vec<nexum_acp::runtime::stable_turn::StableFlow>,
    terminals: Vec<nexum_acp::session::terminal::TerminalState>,
    loading: Vec<bool>,
    prompt_restoration_counts: Vec<u32>,
    new_session_count: usize,
    prompt_rpc_count: usize,
}

async fn run_isolated_stable_session(prompts: &[&str]) -> StableSessionEvidence {
    use std::sync::{
    ...
    use nexum_acp::{
        runtime::stable_turn::StableTurnPolicy,
        session::{
            event_sink::TransportEventSink,
            one_shot::{OneShotExecutor, OneShotRequest},
        },
        task::TaskEnvelopeV1,
        transport::{types::IncomingMessage, AcpTransport},
    };

    let (mut app, _handle) = App::new_headless(80, 24).await;
    install_test_provider(&app);
    let provider = {
        let config = app.services.nexum_config.read();
        crate::app::agent::LlmProvider::from_config(&config).unwrap()
    };
    let provider_name = provider.display_name().to_string();
    let model_name = provider.model_name().to_string();
    let server = Arc::new(install_test_acp(&mut app));
    let calls = Arc::new(AtomicUsize::new(0));
    assert!(!prompts.is_empty());
    let received = Arc::new(Mutex::new(Vec::<TaskEnvelopeV1>::new()));
    let observed_flows = Arc::new(Mutex::new(Vec::new()));
    let new_session_count = Arc::new(AtomicUsize::new(0));
    let prompt_rpc_count = Arc::new(AtomicUsize::new(0));
    let prompt_rpc_completed = Arc::new(AtomicUsize::new(0));
    let prompts = prompts
        .iter()
    ...
    let server_task = {
        ...
        let observed_flows = observed_flows.clone();
        let new_session_count = new_session_count.clone();
        let prompt_rpc_count = prompt_rpc_count.clone();
        let prompt_rpc_completed = prompt_rpc_completed.clone();
        ...
        tokio::spawn(async move {
            ...
            server
                .send_response(id, Ok(serde_json::json!({"sessionId": "session-e2e"})))
                .await
                .unwrap();
            ...
            for prompt in prompts {
                let Some(IncomingMessage::Request {
                    ...
                    panic!("expected session/prompt");
                };
                assert_eq!(method, "session/prompt");
                prompt_rpc_count.fetch_add(1, Ordering::SeqCst);
                assert_eq!(params["stableProfile"], true);
                let envelope: TaskEnvelopeV1 =
                    serde_json::from_value(params["taskEnvelope"].clone()).unwrap();
                let policy = StableTurnPolicy::stable();
                let flow = policy.validate_envelope(&envelope, &provider).unwrap();
                received.lock().unwrap().push(envelope);
                observed_flows.lock().unwrap().push(flow);

                let sink = Arc::new(TransportEventSink::new(server.clone()));
                let result = OneShotExecutor::new(policy)
                    .execute(
                        flow,
                        OneShotRequest {
                            model: Box::new(StableE2eModel {
                                calls: calls.clone(),
                            ...
                            content: nexum_agent::messages::MessageContent::text(prompt.clone()),
                            history: vec![],
                            session_id: "session-e2e".into(),
                            system_prompt: "respuesta directa".into(),
                            context_window: 4096,
                            cancel: nexum_agent::agent::AgentCancellationToken::new(),
                            event_sink: sink,
                        },
                    )
                    .await;
                server
                    .send_response(id, Ok(serde_json::json!({"ok": result.ok})))
                    .await
                    .unwrap();
                prompt_rpc_completed.fetch_add(1, Ordering::SeqCst);
            }
        })
    };

    let mut terminals = Vec::with_capacity(prompts.len());
    let mut loading = Vec::with_capacity(prompts.len());
    let mut prompt_restoration_counts = Vec::with_capacity(prompts.len());
    for (turn_index, prompt) in prompts.iter().enumerate() {
        app.submit_message(prompt.clone());
        poll_until_terminal(&mut app).await;
        for _ in 0..100 {
            if prompt_rpc_completed.load(Ordering::SeqCst) > turn_index {
                break;
            }
            tokio::task::yield_now().await;
            app.poll_agent();
        ...
        assert_eq!(
            prompt_rpc_completed.load(Ordering::SeqCst),
            turn_index + 1,
            "prompt RPC did not complete before the next turn"
        );
        terminals.push(
            app.session_mgr
                .current()
                .agent
                .turn_terminal
                .expect("turn must be terminal"),
        );
        loading.push(app.session_mgr.current().ui.loading);
        prompt_restoration_counts.push(app.session_mgr.current().agent.prompt_restoration_count);
    }
    ...
    let envelopes = received.lock().unwrap().clone();
    let flows = observed_flows.lock().unwrap().clone();
    StableSessionEvidence {
        app,
        request_count: calls.load(Ordering::SeqCst),
        envelopes,
        flows,
        terminals,
        loading,
        prompt_restoration_counts,
        new_session_count: new_session_count.load(Ordering::SeqCst),
        prompt_rpc_count: prompt_rpc_count.load(Ordering::SeqCst),
    }
}

async fn run_isolated_stable_e2e(
    prompt: &str,
) -> (
    App,
    usize,
    nexum_acp::task::TaskEnvelopeV1,
    nexum_acp::runtime::stable_turn::StableFlow,
) {
    let mut evidence = run_isolated_stable_session(&[prompt]).await;
    (
        evidence.app,
        evidence.request_count,
        evidence.envelopes.remove(0),
        evidence.flows.remove(0),
    )
}

#[tokio::test]
async fn new_session_failure_restores_prompt_once() {
    use nexum_acp::transport::{types::IncomingMessage, AcpTransport};
    let (mut app, _handle) = App::new_headless(80, 24).await;
    install_test_provider(&app);
    let server = install_test_acp(&mut app);
    let task = tokio::spawn(async move {
        ...
        server
            .send_response(
                id,
                Err(nexum_acp::transport::types::AcpError::new(
                    -32000,
                    "new failed",
                )),
            )
...
#[tokio::test]
async fn load_session_failure_restores_prompt_once() {
    use nexum_acp::transport::{types::IncomingMessage, AcpTransport};
    let (mut app, _handle) = App::new_headless(80, 24).await;
    install_test_provider(&app);
    app.session_mgr.current_mut().current_thread_id = Some("thread-existing".into());
    let server = install_test_acp(&mut app);
    let task = tokio::spawn(async move {
        let Some(IncomingMessage::Request { id, method, .. }) = server.recv().await else {
...
#[tokio::test]
async fn prompt_rpc_failure_restores_prompt_once() {
    use nexum_acp::transport::{types::IncomingMessage, AcpTransport};
    let (mut app, _handle) = App::new_headless(80, 24).await;
    install_test_provider(&app);
    let server = install_test_acp(&mut app);
    let task = tokio::spawn(async move {
        for expected in ["session/new", "session/prompt"] {
            let Some(IncomingMessage::Request {
                id, method, params, ..
            }) = server.recv().await
            else {
            ...
            if method == "session/new" {
                server
                    .send_response(id, Ok(serde_json::json!({"sessionId": "session-a"})))
                    .await
                    .unwrap();
            } else {
                assert_eq!(params["stableProfile"], true);
                assert!(params.get("taskEnvelope").is_some());
                server
                    .send_response(
                        id,
                        Err(nexum_acp::transport::types::AcpError::new(
...
#[tokio::test]
async fn unavailable_acp_restores_prompt_once() {
    let (mut app, _handle) = App::new_headless(80, 24).await;
    install_test_provider(&app);
    assert!(app.acp_client.is_none());
...
#[tokio::test]
async fn simple_turn_completes_exactly_once() {
    let (app, request_count, envelope, flow) = run_isolated_stable_e2e("Hola").await;

    assert_eq!(flow, nexum_acp::runtime::stable_turn::StableFlow::OneShot);
    assert_eq!(envelope.sanitized_metadata["route_decision"], "one_shot");
    assert_eq!(envelope.sanitized_metadata["task_classification"], "simple");
    assert_eq!(request_count, 1);
    assert!(envelope.allowed_tools.is_empty());
    assert_eq!(
        app.session_mgr.current().agent.turn_terminal,
        Some(nexum_acp::session::terminal::TerminalState::Completed)
    );
...
#[tokio::test]
async fn second_turn_after_restart_passes() {
    let before_restart = run_isolated_stable_session(&["Hola"]).await;
    let after_restart = run_isolated_stable_session(&["Hola"]).await;

    for evidence in [before_restart, after_restart] {
        assert_eq!(evidence.new_session_count, 1);
        assert_eq!(evidence.request_count, 1);
        assert_eq!(evidence.prompt_rpc_count, 1);
        assert_eq!(
            evidence.flows,
            vec![nexum_acp::runtime::stable_turn::StableFlow::OneShot]
        );
        assert_eq!(
            evidence.terminals,
            vec![nexum_acp::session::terminal::TerminalState::Completed]
        );
        assert_eq!(evidence.loading, vec![false]);
        assert_eq!(evidence.prompt_restoration_counts, vec![1]);
    }
}

#[tokio::test]
async fn controlled_demo_canary_30_simple_turns_in_10_reused_sessions() {
    let mut successful_simple_turns = 0_usize;
    let mut new_sessions = 0_usize;
    let mut second_turns = 0_usize;
    let mut provider_requests = 0_usize;
    let mut prompt_rpcs = 0_usize;

    for _ in 0..10 {
        let evidence = run_isolated_stable_session(&["Hola", "Hola", "Hola"]).await;
        new_sessions += evidence.new_session_count;
        second_turns += evidence.terminals.len().saturating_sub(1);
        provider_requests += evidence.request_count;
        prompt_rpcs += evidence.prompt_rpc_count;

        assert_eq!(
            evidence.flows,
            vec![nexum_acp::runtime::stable_turn::StableFlow::OneShot; 3]
        );
        assert_eq!(
            evidence.terminals,
            vec![nexum_acp::session::terminal::TerminalState::Completed; 3]
        );
        assert_eq!(evidence.loading, vec![false; 3]);
        assert_eq!(evidence.prompt_restoration_counts, vec![1; 3]);
        assert_eq!(evidence.request_count, 3);
        assert_eq!(evidence.prompt_rpc_count, 3);
        successful_simple_turns += evidence.terminals.len();
    }

    assert_eq!(successful_simple_turns, 30);
    assert_eq!(new_sessions, 10);
    assert_eq!(second_turns, 20);
    assert_eq!(provider_requests, 30);
    assert_eq!(prompt_rpcs, 30);
}

#[tokio::test]
async fn controlled_demo_canary_five_cancellations_restore_once() {
    let (mut app, _handle) = App::new_headless(80, 24).await;

    for turn in 0..5 {
        let prompt = format!("cancelación-{turn}");
        prepare_loading_turn(&mut app, &prompt);
        app.handle_acp_notification(crate::acp_client::client::AcpNotification::TurnTerminal {
            state: nexum_acp::session::terminal::TerminalState::Cancelled,
            message: Some("cancelled".into()),
        });
        app.handle_acp_notification(crate::acp_client::client::AcpNotification::AgentDone {
            session_id: "duplicate-terminal".into(),
        });

        assert_eq!(
            app.session_mgr.current().agent.turn_terminal,
            Some(nexum_acp::session::terminal::TerminalState::Cancelled)
        );
        assert!(!app.session_mgr.current().ui.loading);
        assert_eq!(app.session_mgr.current().ui.textarea.lines(), &[prompt]);
        assert_eq!(app.session_mgr.current().agent.prompt_restoration_count, 1);
    }
}

#[tokio::test]
async fn isolated_e2e_advanced_task_is_rejected_without_provider_request() {
    for prompt in [
        ...
        let (app, request_count, envelope, flow) = run_isolated_stable_e2e(prompt).await;

        assert_eq!(
            flow,
            nexum_acp::runtime::stable_turn::StableFlow::RejectedByPolicy
        );
        assert_eq!(
            envelope.sanitized_metadata["route_decision"],
            "rejected_by_policy"
        );
        assert_eq!(
            envelope.sanitized_metadata["task_classification"],
            "advanced"
        );
        assert_eq!(request_count, 0, "provider llamado para {prompt:?}");
        assert!(envelope.allowed_tools.is_empty());
        assert_eq!(
            app.session_mgr.current().agent.turn_terminal,
...
#[tokio::test]
async fn isolated_e2e_transport_close_is_one_failed_terminal() {
    let (mut app, _handle) = App::new_headless(80, 24).await;
    prepare_loading_turn(&mut app, "Hola");
    let server = install_test_acp(&mut app);
    drop(server);
...
// ─── Sprint copy-parcial 2026-07-07: modo selección de texto ─────────────────

/// Con selection_mode OFF (default), el footer muestra el atajo de copy y el
/// de seleccionar; sin el mensaje de "arrastrá para seleccionar".
#[tokio::test]
async fn test_footer_normal_muestra_copiar_y_seleccionar() {
    let (mut app, mut handle) = App::new_headless(120, 24).await;
    assert!(!app.global_ui.selection_mode, "default: selection_mode off");
    handle
        .terminal
        .draw(|f| crate::ui::main_ui::render(f, &mut app))
        .unwrap();
    let snap = handle.snapshot().join("\n");
    assert!(
        snap.contains("Ctrl+C"),
        "footer normal muestra Ctrl+C: \n{snap}"
    );
    assert!(
        snap.contains("Ctrl+S"),
        "footer normal muestra el atajo de seleccionar (Ctrl+S): \n{snap}"
    );
    assert!(
        !snap.contains("arrastrá para seleccionar"),
        "en modo normal NO se muestra el mensaje de selección activa: \n{snap}"
    );
}

/// Con selection_mode ON, el footer muestra el mensaje claro de cómo
/// seleccionar y cómo volver.
#[tokio::test]
async fn test_footer_modo_seleccion_muestra_instrucciones() {
    let (mut app, mut handle) = App::new_headless(120, 24).await;
    ...
    assert!(
        snap.contains("arrastrá para seleccionar"),
        "modo selección muestra cómo seleccionar: \n{snap}"
    );
    assert!(
        snap.contains("volver a Nexum"),
        "modo selección muestra cómo volver: \n{snap}"
    );
}

// ─── Sprint public-demo 2026-07-07: NEXUM_PUBLIC_DEMO master switch ─────────

/// NEXUM_PUBLIC_DEMO=1 fuerza predictions off aunque NEXUM_PREDICTIONS=on esté
/// seteado (defensa contra config previa de la máquina contaminando la demo).
#[tokio::test]
async fn test_public_demo_fuerza_predictions_off_pese_a_override() {
    let _g = PROVIDER_ENV_TEST_LOCK.lock().unwrap();
    let prev_demo = std::env::var("NEXUM_PUBLIC_DEMO").ok();
    let prev_pred = std::env::var("NEXUM_PREDICTIONS").ok();
    std::env::set_var("NEXUM_PUBLIC_DEMO", "1");
    std::env::set_var("NEXUM_PREDICTIONS", "on"); // override local que debe ser ignorado

    assert!(
        !crate::ui::composer::predictions_enabled(),
        "public demo debe ganarle a NEXUM_PREDICTIONS=on"
    );
...
/// NEXUM_PUBLIC_DEMO=1 fuerza la active prompt bar off pese a un override.
#[tokio::test]
async fn test_public_demo_fuerza_active_prompt_bar_off_pese_a_override() {
    let _g = PROVIDER_ENV_TEST_LOCK.lock().unwrap();
    let prev_demo = std::env::var("NEXUM_PUBLIC_DEMO").ok();
    let prev_bar = std::env::var("NEXUM_ACTIVE_PROMPT_BAR").ok();
    std::env::set_var("NEXUM_PUBLIC_DEMO", "1");
    std::env::set_var("NEXUM_ACTIVE_PROMPT_BAR", "on");

    assert!(
        !crate::ui::main_ui::sticky_header::active_prompt_bar_enabled(),
        "public demo debe ganarle a NEXUM_ACTIVE_PROMPT_BAR=on"
    );
