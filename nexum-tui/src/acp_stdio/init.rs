...
use nexum_middlewares::prelude::{PermissionMode, SharedPermissionMode};
...
pub(super) async fn init_stdio_context(cwd: String) -> anyhow::Result<Arc<StdioContext>> {
    let _telemetry = nexum_agent::telemetry::init_tracing("nexum-acp");
    ...
    let cron_control = CronControlClient::unavailable();
    ...
    let thread_store: Arc<dyn ThreadStore> =
        match nexum_tui::thread::SqliteThreadStore::default_path().await {
            Ok(store) => Arc::new(store),
            Err(_) => Arc::new(
                nexum_tui::thread::SqliteThreadStore::fallback_path()
                    .await
    ...
    // 初始化 Langfuse
    let langfuse_session = if let Some(config) = nexum_acp::langfuse::LangfuseConfig::from_env() {
    ...
    let session_manager = {
        let nexum_config_snapshot = nexum_config.clone();
        let provider_snapshot = provider.clone();
        nexum_acp::session::SessionManager::new(
            thread_store.clone(),
            provider_snapshot,
            Arc::new(nexum_config_snapshot),
    ...
    Ok(Arc::new(StdioContext {
        ...
        nexum_config: RwLock::new(nexum_config),
        permission_mode,
        cron_control,
        mcp_pool,
        channel_state: None,
