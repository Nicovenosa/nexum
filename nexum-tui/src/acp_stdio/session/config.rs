...
use nexum_acp::session::state_builders::{apply_thinking_effort, parse_permission_mode};
    ...
    match &req.value {
        agent_client_protocol_schema::SessionConfigOptionValue::ValueId { value } => {
            ...
            match config_id {
                ...
                "thinking_effort" => {
                    apply_thinking_effort(&ctx.nexum_config, v);
...
pub(crate) async fn handle_update_config(
    ...
    let new_cfg: nexum_tui::config::NexumConfig = serde_json::from_value(config_val)
    ...
    *ctx.nexum_config.write() = new_cfg.clone();

    if let Some(p) = nexum_tui::app::agent::LlmProvider::from_config(&new_cfg) {
        tracing::info!(
            model = %p.model_name(),
