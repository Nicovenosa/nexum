...
use nexum_agent::{
    agent::AgentCancellationToken, messages::ContentBlock as NexumContentBlock,
...
pub(crate) async fn handle_prompt(
    ...
    // Convert ACP SDK ContentBlocks to nexum-agent MessageContent
    let content = if req.prompt.is_empty() {
        MessageContent::text("")
    } else {
        let blocks: Vec<NexumContentBlock> = req
            .prompt
            .iter()
            .filter_map(|b| match b {
                agent_client_protocol::schema::ContentBlock::Text(t) => {
                    Some(NexumContentBlock::text(&t.text))
                }
                agent_client_protocol::schema::ContentBlock::Image(img) => {
                    Some(NexumContentBlock::image_base64(&img.mime_type, &img.data))
                }
