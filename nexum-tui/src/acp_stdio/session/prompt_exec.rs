...
use nexum_acp::session::{event_sink::StdioEventSink, executor};
use nexum_agent::{agent::AgentCancellationToken, messages::MessageContent};
...
pub(crate) struct PromptExecParams {
    ...
    pub history: Vec<nexum_agent::messages::BaseMessage>,
    pub session_start_source: Option<String>,
    pub history_len: usize,
    pub cancel: AgentCancellationToken,
    pub pool: Arc<parking_lot::Mutex<nexum_acp::session::agent_pool::AgentPool>>,
...
pub(crate) async fn run(params: PromptExecParams) {
    ...
    let broker: Arc<dyn nexum_agent::interaction::UserInteractionBroker> =
    ...
    let nexum_config_snapshot = Arc::new(ctx.nexum_config.read().clone());
    ...
    let result = executor::execute_prompt(executor::PromptExecutionContext {
        ...
        cron_control: ctx.cron_control.clone(),
