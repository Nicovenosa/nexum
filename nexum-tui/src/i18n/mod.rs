...
const ES_AR_FTL: &str = include_str!("../../locales/es-AR/main.ftl");
...
impl LcRegistry {
    pub fn new(lang: Option<&str>) -> Self {
        ...
        bundles.insert("es-AR".to_string(), Self::create_bundle("es-AR", ES_AR_FTL));

        let current_lang = match lang.map(normalize_lang) {
    ...
    fn create_bundle(lang: &str, source: &str) -> FluentBundle<FluentResource> {
        let langid = match lang {
            ...
            "es-AR" => unic_langid::langid!("es-AR"),
