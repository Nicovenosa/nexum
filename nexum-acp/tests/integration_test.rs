//! Integration tests for nexum-acp.
//!
...
async fn test_transport_full_roundtrip() {
    let (client, server) = nexum_acp::transport::mpsc::mpsc_transport_pair();
    ...
    // Client: send request
    use nexum_acp::transport::AcpTransport;
    let result = client
...
async fn test_event_mapper_tool_start() {
    use nexum_acp::event::map_event;
    use nexum_agent::{agent::events::AgentEvent as ExecutorEvent, messages::MessageId};
...
fn test_event_mapper_todo_update_maps_to_plan() {
    ...
    use nexum_acp::event::map_event;
    use nexum_agent::agent::events::{AgentEvent as ExecutorEvent, TodoEntry, TodoStatus};
