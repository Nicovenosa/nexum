...
#[test]
fn test_local_micro_preserva_identidad_locale_objetivo_y_salida_sin_secciones_prohibidas() {
    let result = build_system_prompt(
        None,
        "/tmp",
    ...
    assert!(
        result.contains("You are Nexum Agent"),
        "debe preservar la identidad"
    );
    assert!(
        result.contains("Always respond in es-AR"),
        "debe preservar el locale"
    );
    assert!(
        result.contains("user's stated objective"),
        "debe preservar el objetivo"
    );
    assert!(
        result.contains("requested output"),
        "debe preservar la salida"
    );
    for prohibited in ["subagent", "skills", "project", "docs", "tool"] {
        assert!(
            !normalized.contains(prohibited),
            "LocalMicro no debe incluir la sección prohibida {prohibited}: {result}"
        );
    }
}
