...
//!
//! Sections are loaded from nexum-tui's `prompts/sections/` directory using
//! `include_str!` with paths relative to the nexum-acp crate root.

use nexum_middlewares::AgentOverrides;
...
impl PromptFeatures {
    ...
    pub fn detect() -> Self {
        Self {
            // SPEC-SECURITY-001 (GAP-P0): HITL activo por defecto; solo un
            // override YOLO explícito (true/1/on/yes) lo desactiva.
            hitl_enabled: !nexum_middlewares::hitl::is_yolo_mode(),
    ...
    /// Prompt mínimo reservado para el envelope local E2E explícito.
    pub fn local_micro() -> Self {
        Self {
            hitl_enabled: false,
...
fn format_available_agents(cwd: &str, extra_agent_dirs: &[std::path::PathBuf]) -> String {
    let agents = nexum_middlewares::scan_agents_detailed(cwd, extra_agent_dirs);
    if agents.is_empty() {
...
pub fn build_system_prompt(
    ...
    // include_str! 路径相对于 source file，从 nexum-acp/src/prompt/mod.rs 出发
    let static_sections: &[&str] = &[
        include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nexum-tui/prompts/sections/01_intro.md"
        )),
        include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nexum-tui/prompts/sections/02_system.md"
        )),
        include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nexum-tui/prompts/sections/03_doing_tasks.md"
        )),
        include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nexum-tui/prompts/sections/04_actions.md"
        )),
        include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nexum-tui/prompts/sections/05_using_tools.md"
        )),
        include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nexum-tui/prompts/sections/06_tone_style.md"
        )),
    ];

    // 动态段落（含环境变量占位符、feature-gated 段落）—— 边界标记之后，不参与缓存
    let mut dynamic_sections: Vec<&str> = Vec::new();
    if !features.local_micro {
        dynamic_sections.push(include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nexum-tui/prompts/sections/07_env.md"
        )));
        dynamic_sections.push(include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../nexum-tui/prompts/sections/14_system_reminder.md"
        )));
        if features.hitl_enabled {
            dynamic_sections.push(include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../nexum-tui/prompts/sections/10_hitl.md"
            )));
        }
        if features.subagent_enabled {
            dynamic_sections.push(include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../nexum-tui/prompts/sections/11_subagent.md"
            )));
        }
        if features.cron_enabled {
            dynamic_sections.push(include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../nexum-tui/prompts/sections/12_cron.md"
            )));
        }
        if features.skills_enabled {
            dynamic_sections.push(include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../nexum-tui/prompts/sections/13_skills.md"
            )));
        }
        if features.channel_enabled {
            dynamic_sections.push(include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../nexum-tui/prompts/sections/15_channel.md"
            )));
        }
    }
    ...
    if features.local_micro {
        result.push_str(
            "You are Nexum Agent. Follow the user's stated objective exactly. Return only the requested output.",
        );
    } else {
