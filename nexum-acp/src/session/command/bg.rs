...
use nexum_agent::agent::events::AgentEvent as ExecutorEvent;
use nexum_agent::messages::MessageId;
use nexum_middlewares::prelude::*;
use nexum_middlewares::tools::BoxToolWrapper;
...
impl AgentCommand for BgCommand {
    ...
    async fn execute(&self, ctx: CommandContext) -> CommandResult {
        ...
        // 构造 LLM 实例（从 nexum_config 构建）
        let llm: Box<dyn nexum_agent::agent::react::ReactLLM + Send + Sync> =
            match LlmProvider::from_config(&ctx.nexum_config) {
                Some(provider) => Box::new(nexum_agent::llm::BaseModelReactLLM::new(
                    provider.into_model(),
                )),
                None => {
                    ctx.event_sink
        ...
        let parent_tools: Arc<Vec<Arc<dyn nexum_agent::tools::BaseTool>>> = {
            let mut tools: Vec<Box<dyn nexum_agent::tools::BaseTool>> =
            ...
            Arc::new(
                tools
                    .into_iter()
                    .map(|t| Arc::new(BoxToolWrapper(t)) as Arc<dyn nexum_agent::tools::BaseTool>)
                    .collect(),
            )
        };
        ...
        let spawned = match nexum_middlewares::subagent::spawner::spawn_background_fork(
            nexum_middlewares::subagent::spawner::BgForkConfig {
                prompt: prompt.clone(),
                ...
                bg_event_sender,
                bg_registry,
                fork_directive_kind: nexum_middlewares::subagent::spawner::BgForkDirectiveKind::Bg,
