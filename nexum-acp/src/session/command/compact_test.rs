...
fn make_ctx(
    ...
    super::super::CommandContext {
        ...
        history,
        cwd: "/tmp".to_string(),
        nexum_config: Arc::new(Default::default()),
        auxiliary_model: None,
        event_sink: sink,
        args: String::new(),
        cancel_token: nexum_agent::agent::AgentCancellationToken::new(),
        thread_store: None,
        thread_id: None,
...
// 注意：extract_file_info / extract_skill_names 来自 nexum-agent crate，
// 通过 `use super::*` 间接可见（super::pipeline 重导出？否）。
// 这里显式引用 nexum_agent::agent::compact 以保持独立可读。
use nexum_agent::agent::compact::{extract_file_info, extract_skill_names};
...
async fn test_contract_compact_output_starts_with_human_summary() {
    ...
    assert!(
        first_text.contains(nexum_agent::agent::compact::CONTINUATION_HINT),
