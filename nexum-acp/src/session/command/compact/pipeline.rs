...
//!   → (emit_completed)
//!
// [TRAP] 配置覆盖顺序：nexum_config.config.compact.clone().unwrap_or_default() 之后
...
/// Compact 配置（从 nexum_config 提取并应用 env overrides 后的快照）。
pub type CompactConfig = nexum_agent::agent::compact::CompactConfig;
...
pub fn load_compact_config(nexum_config: &crate::provider::NexumConfig) -> CompactConfig {
...
pub async fn run_pipeline(ctx: CommandContext) -> PipelineOutcome {
    ...
    // 阶段 2: 加载 compact 配置
    let compact_config = load_compact_config(&nexum_config);
