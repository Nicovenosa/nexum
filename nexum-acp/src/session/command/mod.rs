...
use nexum_agent::{llm::BaseModel, messages::BaseMessage};
...
pub struct CommandContext {
    ...
    pub bg_event_sender:
        Option<tokio::sync::mpsc::UnboundedSender<nexum_agent::agent::events::AgentEvent>>,
    /// 后台任务注册中心（BgCommand 等 Immediate 命令依赖）。
    /// Option 因为非 bg 命令路径不需要。BgCommand 总是 expect 它是 Some。
    pub bg_registry: Option<Arc<nexum_middlewares::subagent::BackgroundTaskRegistry>>,
