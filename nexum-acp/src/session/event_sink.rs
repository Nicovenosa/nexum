...
//! prompt execution logic can live in `nexum-acp`.
...
pub trait EventSink: Send + Sync {
    ...
    /// Stable turns use an explicit typed terminal notification.
    async fn push_terminal(&self, session_id: &str, _state: TerminalState, _message: Option<&str>) {
        self.push_done(session_id).await;
    }
}
...
impl EventSink for TransportEventSink {
    ...
    async fn push_terminal(&self, session_id: &str, state: TerminalState, message: Option<&str>) {
        debug!(session_id = %session_id, ?state, "EventSink: sending stable turn terminal");
        if let Err(error) = self
            ...
            .await
        {
            error!(session_id = %session_id, %error, "EventSink: turn terminal send failed");
        }
    }
}
