...
use crate::provider::{LlmProvider, NexumConfig, ProviderConfig, ProviderModels};
...
fn make_session_manager(tmp: &tempfile::TempDir) -> SessionManager {
    ...
    nexum_config.config.providers = vec![make_provider_config("a", "gpt-4o")];
    let provider = LlmProvider::from_config(&nexum_config).unwrap();
    SessionManager::new(
        thread_store,
        provider,
        Arc::new(nexum_config),
