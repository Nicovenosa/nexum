...
use nexum_agent::goal::{GoalStatus, GoalStore, ThreadGoal};
...
impl GoalState {
    ...
    ) -> Result<(), nexum_agent::goal::GoalStoreError> {
    ...
    /// clear：清空 goal
    pub async fn clear(&self) -> Result<(), nexum_agent::goal::GoalStoreError> {
        let (thread_id, store) = {
            let mut guard = self.inner.write();
            guard.goal = None;
...
impl nexum_agent::goal::GoalStateView for GoalState {
    fn snapshot(&self) -> nexum_agent::goal::GoalViewSnapshot {
        let snap = self.snapshot();
        nexum_agent::goal::GoalViewSnapshot {
...
#[async_trait]
impl nexum_agent::goal::GoalController for GoalState {
    ...
    fn snapshot(&self) -> nexum_agent::goal::GoalViewSnapshot {
        nexum_agent::goal::GoalStateView::snapshot(self)
    }
}
