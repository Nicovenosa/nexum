...
use crate::provider::{LlmProvider, NexumConfig, ThinkingConfig};
...
/// Apply a thinking effort level to `NexumConfig` (writes through `RwLock`).
pub fn apply_thinking_effort(nexum_config: &RwLock<NexumConfig>, effort: &str) {
    let mut cfg = nexum_config.write();
...
pub fn build_config_options(
    ...
    let active_alias = nexum_config.config.active_alias.clone();
    let active_provider = nexum_config.config.providers.iter().find(|prov| {
        prov.id == nexum_config.config.active_provider_id
            || nexum_config.config.active_provider_id.is_empty()
    });
