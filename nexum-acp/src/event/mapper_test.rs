...
#[test]
fn test_llm_call_end_no_usage_still_records_execution() {
    let event = ExecutorEvent::LlmCallEnd {
        step: 1,
    ...
    match &mapped[0].updates[0] {
        SessionUpdate::UsageUpdate(usage) => {
            ...
            assert_eq!(meta["httpStatus"], 200);
            assert_eq!(meta["requestSent"], true);
            assert_eq!(meta["responseReceived"], true);
        }
