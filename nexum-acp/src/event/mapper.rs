...
pub fn map_event(event: &ExecutorEvent, context_window: u32) -> Vec<MappedEvent> {
    match event {
        ...
        ExecutorEvent::LlmCallEnd {
            ...
            let input_tokens = usage.as_ref().map(|value| value.input_tokens).unwrap_or(0);
            let output_tokens = usage.as_ref().map(|value| value.output_tokens).unwrap_or(0);
            meta.insert("inputTokens".into(), serde_json::json!(input_tokens));
            meta.insert("outputTokens".into(), serde_json::json!(output_tokens));
            if let Some(v) = usage
                .as_ref()
            ...
            meta.insert("httpStatus".into(), serde_json::json!(200));
            meta.insert("requestSent".into(), serde_json::json!(true));
            meta.insert("responseReceived".into(), serde_json::json!(true));
        ...
        // ── Filtered: no output ───────────────────────────────────────────────────
        ExecutorEvent::MessageAdded(_) | ExecutorEvent::LlmCallStart { .. } => {
