...
use nexum_lsp::config::{LspConfigSource, LspServerConfig};
...
/// Plugin-derived runtime inputs shared by every ACP host. Keeping the hooks
/// alongside plugin data prevents each host from reconstructing a subtly
...
impl RuntimePluginSnapshot {
    /// Adds settings-derived hooks to already loaded plugin data without
    /// re-reading plugin manifests in a frontend that also needs commands/UI.
    pub fn from_plugin_data(plugin_data: &PluginLoadResult, cwd: &str) -> Self {
        let mut hook_groups = Vec::new();
        if !plugin_data.all_hooks.is_empty() {
            hook_groups.push(plugin_data.all_hooks.clone());
        }
        let global_hooks = crate::hooks::loader::load_global_settings_hooks();
        if !global_hooks.is_empty() {
            hook_groups.push(global_hooks);
        }
        let local_hooks = crate::hooks::loader::load_settings_local_hooks(cwd);
        if !local_hooks.is_empty() {
            hook_groups.push(local_hooks);
        }
        let hooks = hook_groups.iter().flatten().cloned().collect();

        Self {
            skill_roots: plugin_data.all_skill_roots.clone(),
            agent_dirs: plugin_data.all_agent_dirs.clone(),
            hooks,
            hook_groups,
            lsp_servers: plugin_data
                .all_lsp_servers
                .iter()
                .filter(|server| server.disabled != Some(true))
                .cloned()
                .collect(),
        }
    }
...
/// Loads the plugin and settings-derived inputs consumed by an ACP runtime.
/// This performs discovery only; it never starts MCP, LSP, hooks, or tools.
pub fn load_runtime_plugin_snapshot(claude_dir: &Path, cwd: &str) -> RuntimePluginSnapshot {
    let plugin_data = load_enabled_plugins_aggregated(claude_dir);
    RuntimePluginSnapshot::from_plugin_data(&plugin_data, cwd)
}
