...
async fn test_micro_compact_once_per_prompt() {
    ...
    for i in 0..8 {
        state.add_message(BaseMessage::ai_with_tool_calls(
            nexum_agent::messages::MessageContent::text("using tool"),
            vec![nexum_agent::messages::ToolCallRequest::new(
                format!("tc{}", i),
                "Bash",
                serde_json::json!({}),
            )],
    ...
    state
        .token_tracker_mut()
        .accumulate(&nexum_agent::llm::types::TokenUsage {
            input_tokens: 800,
            output_tokens: 100,
...
fn test_do_full_compact_preserves_system_prefix() {
    ...
    let original_messages = [
        ...
        BaseMessage::ai_with_tool_calls(
            nexum_agent::messages::MessageContent::text("assistant response"),
            vec![nexum_agent::messages::ToolCallRequest::new(
                "tc1",
                "Bash",
...
async fn test_full_compact_cancel_restores_own_messages() {
    ...
    let original: Vec<BaseMessage> = vec![
        ...
        BaseMessage::ai_with_tool_calls(
            nexum_agent::messages::MessageContent::text("using bash"),
            vec![nexum_agent::messages::ToolCallRequest::new(
                "tc1",
                "Bash",
...
async fn test_full_compact_error_restores_own_messages() {
    ...
    let original: Vec<BaseMessage> = vec![
        ...
        BaseMessage::ai_with_tool_calls(
            nexum_agent::messages::MessageContent::text("using tool"),
            vec![
                nexum_agent::messages::ToolCallRequest::new(
                    "t1",
                    "Read",
