// 基础设施（trait / registry / context / matcher / format）从 nexum-agent re-export
// 避免循环依赖：nexum-agent 不依赖 nexum-middlewares，所以核心类型定义在底层
pub use nexum_agent::error_suggest::{
...
// Suggester 测试入口（测试文件留在 nexum-middlewares 因为它们测试 nexum-middlewares 的 suggesters）
#[cfg(test)]
