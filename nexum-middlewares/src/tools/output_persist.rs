...
/// 当输出被截断时，将完整内容写入临时文件。
/// 返回追加到截断信息后的提示字符串。
/// 文件路径：`{temp_dir}/nexum-tool-output-{uuid}.txt`
pub fn persist_truncated_output(full_content: &str) -> String {
    let id = uuid::Uuid::new_v4();
    // ALLOW justificado: el archivo lleva UUID en el nombre (línea de abajo),
    // así que no hay recurso compartido. Es salida truncada de una tool,
    // efímera por definición.
    #[allow(clippy::disallowed_methods)]
    let dir = env::temp_dir();
    let file_name = format!("nexum-tool-output-{id}.txt");
...
mod tests {
    ...
    fn test_persist_writes_file_and_returns_hint() {
        ...
        // 提示应包含文件名
        assert!(
            hint.contains("nexum-tool-output-"),
