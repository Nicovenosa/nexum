    ...
    async fn test_invalid_json_returns_err() {
        ...
        let result = tool.invoke(serde_json::Value::Null, nexum_agent::tools::ToolContext::new(&[], ".")).await;
    ...
    async fn test_missing_questions_key_returns_err() {
        let tool = make_tool(make_answer(&[], None));
        let result = tool.invoke(serde_json::json!({}), nexum_agent::tools::ToolContext::new(&[], ".")).await;
    ...
    async fn test_valid_single_question_parsed() {
        ...
        let result = tool.invoke(single_question_input(), nexum_agent::tools::ToolContext::new(&[], ".")).await.unwrap();
    ...
    async fn test_unexpected_response_type() {
        use nexum_agent::interaction::ApprovalDecision;
        let response = InteractionResponse::Decisions(vec![ApprovalDecision::Approve { source: None }]);
        let tool = make_tool(response);
        let result = tool.invoke(single_question_input(), nexum_agent::tools::ToolContext::new(&[], ".")).await;
