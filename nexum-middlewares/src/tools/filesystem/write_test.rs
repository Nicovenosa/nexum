    ...
    async fn test_write_file_creates_new() {
        ...
        tool.invoke(serde_json::json!({"file_path": "new.txt", "content": "hello"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_write_file_overwrites_existing() {
        ...
        let tool = WriteFileTool::new(dir.path().to_str().unwrap());
        tool.invoke(serde_json::json!({"file_path": "f.txt", "content": "new"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_write_file_creates_parent_dirs() {
        let dir = tempfile::tempdir().unwrap();
        let tool = WriteFileTool::new(dir.path().to_str().unwrap());
        tool.invoke(serde_json::json!({"file_path": "sub/dir/file.txt", "content": "deep"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_write_file_missing_content_param() {
        let dir = tempfile::tempdir().unwrap();
        let tool = WriteFileTool::new(dir.path().to_str().unwrap());
        let result = tool.invoke(serde_json::json!({"file_path": "f.txt"}), nexum_agent::tools::ToolContext::new(&[], ".")).await;
    ...
    async fn test_write_file_success_message() {
        ...
        let tool = WriteFileTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"file_path": "msg.txt", "content": "x"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_write_file_no_tmp_residual() {
        let dir = tempfile::tempdir().unwrap();
        let tool = WriteFileTool::new(dir.path().to_str().unwrap());
        tool.invoke(serde_json::json!({"file_path": "clean.txt", "content": "data"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_write_file_error_propagates() {
        ...
        let _result = tool
            .invoke(serde_json::json!({"file_path": "sub/nope.txt", "content": "x"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
        #[cfg(unix)]
    ...
    async fn test_write_append_false_overwrites() {
        ...
        let tool = WriteFileTool::new(dir.path().to_str().unwrap());
        tool.invoke(serde_json::json!({"file_path": "f.txt", "content": "new", "append": false}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        let content = std::fs::read_to_string(dir.path().join("f.txt")).unwrap();
