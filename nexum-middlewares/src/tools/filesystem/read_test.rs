    ...
    async fn test_read_file_basic() {
        ...
        let result = tool
            .invoke(serde_json::json!({"file_path": "file.txt"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_read_file_not_found() {
        ...
        let tool = ReadFileTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"file_path": "nonexistent.txt"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
    ...
    async fn test_read_file_offset_limit() {
        ...
        let tool = ReadFileTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"file_path": "lines.txt", "offset": 2, "limit": 2}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_read_file_binary_extension() {
        ...
        let tool = ReadFileTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"file_path": "image.png"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_read_file_absolute_path() {
        ...
        let result = tool
            .invoke(serde_json::json!({"file_path": path.to_str().unwrap()}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_read_file_offset_exceeds_length() {
        ...
        let tool = ReadFileTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"file_path": "short.txt", "offset": 999}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
        let err_msg = result.unwrap_err().to_string();
        assert!(
    ...
    async fn test_read_file_too_large() {
        ...
        let tool = ReadFileTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"file_path": "huge.txt"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
        let err_msg = result.unwrap_err().to_string();
        assert!(
    ...
    async fn test_pdf_with_pages_returns_placeholder() {
        let tool = ReadFileTool::new("/tmp");
        let result = tool
            .invoke(serde_json::json!({"file_path": "test.pdf", "pages": "1-5"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_pdf_without_pages_returns_binary() {
        let tool = ReadFileTool::new("/tmp");
        let result = tool
            .invoke(serde_json::json!({"file_path": "test.pdf"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
            result.contains("BINARY FILE DETECTED"),
