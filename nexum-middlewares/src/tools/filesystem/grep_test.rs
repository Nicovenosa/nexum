...
async fn test_grep_files_only() {
    ...
    let tool = GrepTool::new(dir.path().to_str().unwrap());
    let result = tool
            .invoke(serde_json::json!({"pattern": "needle", "output_mode": "files_with_matches", "path": "./"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
...
async fn test_grep_case_insensitive() {
    ...
    let tool = GrepTool::new(dir.path().to_str().unwrap());
    let result = tool
            .invoke(serde_json::json!({"pattern": "NEEDLE", "output_mode": "content", "-i": true, "path": "./"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    assert!(
...
async fn test_grep_glob_filter() {
    ...
    let tool = GrepTool::new(dir.path().to_str().unwrap());
    let result = tool
            .invoke(serde_json::json!({"pattern": "needle", "output_mode": "content", "glob": "*.txt", "path": "./"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
