    ...
    async fn test_folder_create() {
        ...
        let result = tool
            .invoke(serde_json::json!({"operation": "create", "folder_path": "newdir"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_folder_create_recursive() {
        let dir = tempfile::tempdir().unwrap();
        let tool = FolderOperationsTool::new(dir.path().to_str().unwrap());
        tool.invoke(serde_json::json!({"operation": "create", "folder_path": "a/b/c"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_folder_exists_true() {
        ...
        let tool = FolderOperationsTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"operation": "exists", "folder_path": "existing"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_folder_exists_false() {
        ...
        let tool = FolderOperationsTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"operation": "exists", "folder_path": "ghost"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_folder_list() {
        ...
        let tool = FolderOperationsTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"operation": "list", "folder_path": "listed"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_folder_list_truncation_keeps_files() {
        ...
        let tool = FolderOperationsTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"operation": "list", "folder_path": "bigdir"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        // 文件不应被全部丢弃
        assert!(
    ...
    async fn test_folder_list_truncation_persists_full_output() {
        ...
        assert!(result.contains("nexum-tool-output-"), "应包含文件路径: {result}");
    }
