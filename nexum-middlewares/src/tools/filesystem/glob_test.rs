    ...
    async fn test_glob_match_simple() {
        ...
        let result = tool
            .invoke(serde_json::json!({"pattern": "*.rs"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_glob_no_match() {
        ...
        let tool = GlobFilesTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"pattern": "*.go"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_glob_dir_not_found() {
        ...
        let tool = GlobFilesTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"pattern": "*.rs", "path": "nonexistent_dir"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
    ...
    async fn test_glob_soft_warning_prepended_in_output() {
        ...
        let tool = GlobFilesTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"pattern": "*"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        assert!(
    ...
    async fn test_glob_invalid_pattern_returns_error() {
        ...
        let result = tool.invoke(input, nexum_agent::tools::ToolContext::new(&[], ".")).await;
