    ...
    async fn test_edit_file_old_string_not_found() {
        ...
        let tool = EditFileTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"file_path": "f.txt", "old_string": "missing", "new_string": "x"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
    ...
    async fn test_edit_file_empty_old_string_rejected() {
        ...
        let tool = EditFileTool::new(dir.path().to_str().unwrap());
        let result = tool
            .invoke(serde_json::json!({"file_path": "f.txt", "old_string": "", "new_string": "x", "replace_all": true}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
        let err = result.unwrap_err();
        assert!(
