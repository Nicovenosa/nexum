...
use nexum_lsp::pool::LspServerPool;
...
impl LspTool {
    ...
    async fn get_initialized_server(
        &self,
        file_path: &str,
    ) -> Result<Arc<nexum_lsp::client::LspClient>, LspToolError> {
        match self.pool.server_for_file(file_path) {
            ...
            Some(s) => {
                ...
                if matches!(
                    state,
                    nexum_lsp::client::ServerState::Error(_)
                        | nexum_lsp::client::ServerState::Stopped
                ) {
                    // 服务器崩溃或停止，尝试重启
                    tracing::warn!(
                        target: "lsp",
    ...
    async fn get_any_ready_server(&self) -> Result<Arc<nexum_lsp::client::LspClient>, LspToolError> {
        ...
        for s in &servers {
            ...
            if matches!(
                state,
                nexum_lsp::client::ServerState::Error(_) | nexum_lsp::client::ServerState::Stopped
            ) {
                tracing::warn!(
                    target: "lsp",
                    server = %s.name(),
