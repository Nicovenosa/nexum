...
async fn test_invoke_file_not_found() {
    let tool = ArtifactTool::new("/tmp".into());
    let result = tool
        .invoke(serde_json::json!({"file_path": "/nonexistent/file.html", "ttl": "7d"}), nexum_agent::tools::ToolContext::new(&[], "."))
        .await;
...
async fn test_invoke_non_html_extension() {
    ...
    let result = tool
        .invoke(serde_json::json!({"file_path": file_path.to_string_lossy(), "ttl": "7d"}), nexum_agent::tools::ToolContext::new(&[], "."))
        .await;
    assert!(result.is_err());
    let err = result.unwrap_err().to_string();
