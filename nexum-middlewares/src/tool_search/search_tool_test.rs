    ...
    async fn test_invoke_search_returns_results() {
        ...
        let result = tool
            .invoke(json!({"query": "slack message"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_invoke_empty_results() {
        ...
        let result = tool
            .invoke(json!({"query": "nonexistent_tool_xyz"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
        let parsed: Value = serde_json::from_str(&result).unwrap();
    ...
    async fn test_invoke_missing_query() {
        ...
        let result = tool.invoke(json!({}), nexum_agent::tools::ToolContext::new(&[], ".")).await;
