    ...
    async fn test_invoke_executes_deferred_tool() {
        ...
        let result = tool
            .invoke(json!({"tool_name": "CronRegister", "params": {"expression": "* * * * *", "prompt": "test"}}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await
            .unwrap();
    ...
    async fn test_tool_not_found_returns_error() {
        ...
        let result = tool
            .invoke(json!({"tool_name": "UnknownTool", "params": {}}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
    ...
    async fn test_missing_tool_name() {
        ...
        let result = tool.invoke(json!({"params": {}}), nexum_agent::tools::ToolContext::new(&[], ".")).await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    async fn test_missing_params() {
        ...
        let result = tool.invoke(json!({"tool_name": "CronRegister"}), nexum_agent::tools::ToolContext::new(&[], ".")).await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    async fn test_target_tool_error_propagates() {
        ...
        let result = tool
            .invoke(json!({"tool_name": "FailingTool", "params": {}}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
        assert!(result.is_err());
