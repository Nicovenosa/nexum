// ALLOW justificado a nivel de archivo, no suelto.
//
// Los 19 usos de `temp_dir()` acá son el CWD que se le pasa a `BashTool`, no
// archivos con nombre fijo: no se crea ningún recurso que dos procesos puedan
// compartir, así que no aplica el aislamiento por PID. Es la misma categoría
// que el test de pty_session, y distinta de /tmp/zen-threads.db, que sí era un
// recurso compartido y sí era un bug.
//
// Si algún día un test de acá escribe un archivo con nombre fijo, el allow
// deja de cubrirlo honestamente y hay que revisar, no ampliar.
...
fn test_truncate_output_persists_full_content_on_lines_truncation() {
    ...
    assert!(
        result.contains("nexum-tool-output-"),
        "应包含临时文件路径: {result}"
    );
}
...
// ─── PST-2 Fase 10: tree-kill mata nietos en background (Unix) ────────────────

#[cfg(unix)]
#[tokio::test]
async fn test_treekill_mata_nietos_en_background() {
    use nexum_agent::tools::BaseTool;
    let marker = std::env::temp_dir().join(format!("pst-tk-{}", std::process::id()));
    let _ = std::fs::remove_file(&marker);
    let tool = BashTool::new("/tmp");
    // El wrapper lanza un NIETO en background que tras 2s tocaría el marker.
    // Con timeout 300ms, el kill del grupo debe matar también al nieto.
    let cmd = format!("( sleep 2; touch {} ) & sleep 20", marker.display());
    let _ = tool
        .invoke(
            serde_json::json!({"command": cmd, "timeout": 300}),
            nexum_agent::tools::ToolContext::new(&[], "."),
        )
        .await;
    // Esperar más que el nieto (2s): si el árbol se mató, el marker NO existe.
    tokio::time::sleep(std::time::Duration::from_secs(4)).await;
    let survived = marker.exists();
    let _ = std::fs::remove_file(&marker);
    assert!(!survived, "el nieto en background debe morir con el árbol (SEC-007)");
}
