    ...
    async fn test_invoke_missing_server_name() {
        let tool = McpResourceTool::new(empty_pool());
        let result = tool
            .invoke(serde_json::json!({"uri": "file:///test"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
    ...
    async fn test_invoke_missing_uri() {
        let tool = McpResourceTool::new(empty_pool());
        let result = tool
            .invoke(serde_json::json!({"server_name": "test"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
        assert!(result.is_err());
    ...
    async fn test_invoke_server_not_found() {
        let tool = McpResourceTool::new(empty_pool());
        let result = tool
            .invoke(serde_json::json!({"server_name": "nonexistent", "uri": "test://x"}), nexum_agent::tools::ToolContext::new(&[], "."))
            .await;
        assert!(result.is_err());
