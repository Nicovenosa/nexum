...
impl ReactLLM for EchoLLM {
    async fn generate_reasoning(
        ...
        messages: &[BaseMessage],
        _tools: &[&dyn BaseTool],
        _streaming: Option<nexum_agent::llm::types::StreamingContext>,
    ) -> nexum_agent::error::AgentResult<Reasoning> {
        ...
        async fn generate_reasoning(
                ...
                .any(|m| matches!(m, BaseMessage::Tool { .. }))
            {
                ...
                Ok(Reasoning::with_tools(
                    "call missing",
                    vec![nexum_agent::agent::react::ToolCall::new(
                        "id1",
                        "nonexistent",
                        serde_json::json!({}),
                    )],
...
use nexum_agent::agent::State as _;
use tempfile::TempDir;
