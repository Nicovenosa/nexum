...
pub struct BgForkConfig {
    ...
    pub llm: Box<dyn nexum_agent::agent::react::ReactLLM + Send + Sync>,
    /// 最大 ReAct 迭代次数
    pub max_iterations: usize,
    /// 父 Agent 的工具集（子 Agent 继承）
    pub parent_tools: Arc<Vec<Arc<dyn nexum_agent::tools::BaseTool>>>,
    ...
    /// 后台任务完成事件的发送通道（必填）
    pub bg_event_sender: tokio::sync::mpsc::UnboundedSender<nexum_agent::agent::events::AgentEvent>,
    /// 后台任务注册中心
...
pub async fn spawn_background_fork(
    ...
    let llm =
        nexum_agent::llm::RetryableLLM::new(config.llm, nexum_agent::llm::RetryConfig::default());
    ...
    agent_builder = agent_builder.with_event_handler(Arc::new(
        nexum_agent::agent::events::FnEventHandler(move |event: AgentEvent| {
