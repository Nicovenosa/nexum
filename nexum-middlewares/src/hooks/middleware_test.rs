...
async fn test_before_tool_block() {
    ...
    let result = mw
        .before_tool(
            &mut nexum_agent::agent::state::AgentState::new("/test"),
            &tool_call,
        )
        .await;
...
async fn test_before_agent_fires_user_prompt_submit() {
    ...
    let mut state = nexum_agent::agent::state::AgentState::new("/test");
...
async fn test_before_agent_session_start_controlled_by_flag() {
    ...
    let mut state2 = nexum_agent::agent::state::AgentState::new("/test");
...
async fn test_before_tools_batch_fires_permission_request() {
    ...
    use nexum_agent::middleware::Middleware;
...
async fn test_stopfailure_only_fires_on_api_errors() {
    ...
    let err = nexum_agent::error::AgentError::Interrupted;
    ...
    let err = nexum_agent::error::AgentError::MaxIterationsExceeded(500);
    let result = mw.on_error(&mut state, &err).await;
    assert!(
        result.is_ok(),
        "MaxIterationsExceeded should not fire StopFailure"
    ...
    let err = nexum_agent::error::AgentError::ToolRejected {
        tool: "Bash".to_string(),
    ...
    let err = nexum_agent::error::AgentError::ToolExecutionFailed {
        tool: "Bash".to_string(),
    ...
    let err = nexum_agent::error::AgentError::LlmError("rate limit".to_string());
    let result = mw.on_error(&mut state, &err).await;
    assert!(
        result.is_ok(),
    ...
    let err = nexum_agent::error::AgentError::LlmHttpError {
        status: 429,
    ...
    let err = nexum_agent::error::AgentError::MiddlewareError {
...
async fn test_post_tool_batch_fires_after_all_tools() {
    ...
    state.add_message(nexum_agent::messages::BaseMessage::human("test prompt"));
...
async fn test_stop_block_continue_sets_block_continue_field() {
    ...
    let mut state = AgentState::new("/test");
    state.add_message(nexum_agent::messages::BaseMessage::human("test"));

    let output = nexum_agent::agent::react::AgentOutput::new("done", 3);
        ...
        Ok(o) => {
            ...
            if o.block_continue.is_some() {
                ...
                let has_feedback = state.messages().iter().any(|m| {
                    matches!(m, nexum_agent::messages::BaseMessage::Human { .. })
