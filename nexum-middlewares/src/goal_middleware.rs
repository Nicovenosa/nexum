...
pub struct GoalMiddleware {
    controller: Arc<dyn nexum_agent::goal::GoalController>,
...
impl<S: State> Middleware<S> for GoalMiddleware {
    ...
    fn collect_tools(&self, _cwd: &str) -> Vec<Box<dyn nexum_agent::tools::BaseTool>> {
    ...
    async fn after_agent(
        &self,
        state: &mut S,
        output: &nexum_agent::agent::react::AgentOutput,
    ) -> AgentResult<nexum_agent::agent::react::AgentOutput> {
        ...
        if !nexum_agent::goal::is_goal_active(&snap) {
            // 无 goal 或终态 → 重置计数器 + 放行
