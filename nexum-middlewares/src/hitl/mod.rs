...
pub use nexum_agent::hitl::{BatchItem, HitlDecision};
pub use shared_mode::{PermissionMode, SharedPermissionMode};

// ─── YOLO 模式检测 ─────────────────────────────────────────────────────────────

/// Detecta si YOLO está EXPLÍCITAMENTE activado (SPEC-SECURITY-001, GAP-P0).
///
/// Nexum v0.1 es seguro por defecto: la ausencia de configuración NUNCA
/// produce YOLO. Solo un override explícito lo activa.
///
/// - `YOLO_MODE=true|1|on|yes` → YOLO (salta aprobaciones) — override consciente
/// - `YOLO_MODE` ausente, vacío, `false`, `0`, o cualquier otro valor → HITL activo
///
/// Fail-safe: cualquier valor no reconocido cae a HITL (no a YOLO).
...
/// Fingerprint determinístico de una solicitud de aprobación
/// (SPEC-SECURITY-001 / SPEC-TOOLS-001): ata la aprobación a la tool exacta +
/// los argumentos normalizados. Cualquier cambio en el nombre de la tool o en
/// los argumentos produce un fingerprint distinto ⇒ la aprobación previa no
/// aplica y se requiere una nueva. El input se serializa con claves ordenadas
/// (serde_json::Value es un BTreeMap para objetos) para que sea estable.
///
/// El binding fuerte es estructural: el `ToolCall` aprobado es el MISMO objeto
/// que se ejecuta (fluye por `before_tool` → ejecución sin que el modelo lo
/// mute en el medio). Este fingerprint hace esa invariante explícita y
/// verificable, y es la base para cachés/logs de aprobación sin exponer args.
pub fn approval_fingerprint(tool_name: &str, tool_input: &serde_json::Value) -> String {
    use sha2::{Digest, Sha256};
    let mut hasher = Sha256::new();
    hasher.update(tool_name.as_bytes());
    hasher.update([0u8]);
    // to_string de serde_json::Value ordena las claves de los objetos.
    hasher.update(tool_input.to_string().as_bytes());
    format!("{:x}", hasher.finalize())
}

/// Mapea `ApprovalDecision` a `AgentResult<ToolCall>`. El call aprobado es el
/// clone del call mostrado: mismo tool, mismos args (binding estructural).
