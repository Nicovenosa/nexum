...
impl CompactMiddleware {
    ...
    async fn do_full_compact(&self, state: &mut impl State) -> AgentResult<()> {
        ...
        // CONTINUATION_HINT 来自 nexum-agent::agent::compact，与 /compact 命令路径
        // （nexum-acp/src/session/command/compact/invariant.rs）和 TUI 识别层
        // （nexum-tui/src/ui/message_view/build.rs::COMPACT_HINT）三方共享。
