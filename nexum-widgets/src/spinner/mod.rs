...
impl SpinnerState {
    ...
    pub fn set_mode(&mut self, mode: SpinnerMode) {
        ...
        self.verb = match &self.mode {
            SpinnerMode::Thinking => "pensando…".to_string(),
            SpinnerMode::ToolUse => "ejecutando herramienta…".to_string(),
            SpinnerMode::Responding => "generando respuesta…".to_string(),
...
impl<'a> SpinnerWidget<'a> {
    pub fn new(state: &'a SpinnerState) -> Self {
        Self {
            ...
            show_elapsed: true,
            show_tokens: true,
            primary_color: Color::Rgb(0, 212, 170), // Nexum ACCENT #00D4AA
            secondary_color: Color::Rgb(148, 163, 184), // Nexum MUTED #94A3B8
        }
    }
