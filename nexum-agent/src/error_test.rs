...
#[test]
fn test_timeout_no_es_retryable_gap_p1() {
    // GAP-P1 (SPEC-PROVIDERS-001): un silencio del provider (read/connect
    // timeout) NO se reintenta — el cliente ya acota cada intento y reintentar
    // de inmediato solo alarga el hang. Devolver error claro rápido.
    let err = AgentError::LlmError("reqwest timeout exceeded".into());
    assert!(!err.is_retryable());
    // Los transitorios reales sí se reintentan.
    assert!(AgentError::LlmError("connection refused".into()).is_retryable());
    assert!(AgentError::LlmError("overloaded".into()).is_retryable());
}
