...
fn test_fuzzy_top_n_returns_sorted_matches() {
    let candidates: Vec<String> = vec![
        "nexum-agent".into(),
        "nexum-tui".into(),
        "nexum-middlewares".into(),
        "langfuse-client".into(),
    ];
    let result = fuzzy_top_n(&candidates, "nexum", 3);
    assert_eq!(result.len(), 3);
    let names: Vec<&str> = result.iter().map(|s| s.as_str()).collect();
    assert!(names.contains(&"nexum-agent"));
    assert!(names.contains(&"nexum-tui"));
    assert!(names.contains(&"nexum-middlewares"));
}
