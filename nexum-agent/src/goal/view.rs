...
//!
//! GoalState（nexum-acp）实现此 trait，GoalMiddleware（nexum-middlewares）
//! 依赖 trait 而非具体类型，避免 nexum-middlewares → nexum-acp 循环依赖。

use super::model::GoalStatus;

/// 只读快照（与 GoalSnapshot 平行，但定义在 nexum-agent 层避免依赖）
