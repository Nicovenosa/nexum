...
// ─── Micro-fix Ollama: política capability-aware de streaming/timeout ───

const OLLAMA_URL: &str = "http://127.0.0.1:11434/v1";

/// GATE: `ollama_local` usa política non-stream (with_local_cpu_profile).
#[test]
fn test_ollama_local_cpu_profile_disables_streaming() {
    use crate::llm::BaseModel;
    let m = ChatOpenAI::new("ollama", "qwen2.5:1.5b")
        .with_base_url(OLLAMA_URL)
        .with_local_cpu_profile();
    assert!(
        !m.supports_streaming(),
        "el perfil local de CPU debe deshabilitar streaming"
    );
}

/// GATE: los providers remotos conservan su política (streaming on).
/// También cubre `global_streaming_behavior_unchanged`.
#[test]
fn test_remote_openai_keeps_streaming() {
    use crate::llm::BaseModel;
    let m = ChatOpenAI::new("sk-x", "gpt-4o").with_base_url("https://api.openai.com/v1");
    assert!(
        m.supports_streaming(),
        "un provider remoto NO debe verse afectado por el micro-fix de Ollama"
    );
}

/// La primitiva `with_streaming` togglea la capacidad de forma granular.
#[test]
fn test_with_streaming_toggle() {
    use crate::llm::BaseModel;
    let base = ChatOpenAI::new("k", "m");
    assert!(base.supports_streaming(), "default streaming on");
    let off = ChatOpenAI::new("k", "m").with_streaming(false);
    assert!(!off.supports_streaming());
    let on = ChatOpenAI::new("k", "m").with_streaming(false).with_streaming(true);
    assert!(on.supports_streaming());
}

/// GATE: el request a Ollama lleva `stream=false` en modo non-stream, y
/// `stream=true` en streaming. (No cambio global: el flag lo decide el caller.)
#[test]
fn test_request_body_stream_flag() {
    use crate::llm::types::LlmRequest;
    use crate::messages::BaseMessage;
    let adapter = ChatOpenAI::new("ollama", "qwen2.5:1.5b").with_base_url(OLLAMA_URL);
    let req = LlmRequest::new(vec![BaseMessage::human("hola")]);

    let non_stream = super::invoke::build_request_body(&adapter, &req, false);
    assert_eq!(non_stream["stream"], json!(false), "non-stream ⇒ stream=false");

    let streaming = super::invoke::build_request_body(&adapter, &req, true);
    assert_eq!(streaming["stream"], json!(true), "streaming ⇒ stream=true");
}

/// GATE: finish_reason preservado — el mapeo OpenAI→StopReason es estable
/// en el camino non-stream (`invoke()` usa `StopReason::from_openai`).
#[test]
fn test_finish_reason_preserved_mapping() {
    ...
    // finish_reason desconocido se preserva textualmente (no se pierde).
    assert_eq!(
        StopReason::from_openai("algo_raro"),
        StopReason::Other("algo_raro".to_string())
    );
}

/// GATE connect_timeout_test: un endpoint inalcanzable falla RÁPIDO como error
/// de conexión, distinto de esperar el read/generation-timeout. (Sin red externa:
/// puerto local cerrado ⇒ connection refused inmediato.)
#[tokio::test]
async fn test_connect_error_is_fast_and_distinct_from_generation_timeout() {
    use crate::llm::BaseModel;
    use crate::llm::types::LlmRequest;
    use crate::messages::BaseMessage;
    let adapter = ChatOpenAI::new("k", "m").with_base_url("http://127.0.0.1:1/v1");
    let t0 = std::time::Instant::now();
    let res = adapter
        .invoke(LlmRequest::new(vec![BaseMessage::human("hi")]))
        .await;
    let dt = t0.elapsed();
    assert!(res.is_err(), "endpoint inalcanzable debe fallar");
    assert!(
        dt < std::time::Duration::from_secs(10),
        "el fallo de conexión debe ser rápido (no espera el read-timeout): {dt:?}"
    );
}
