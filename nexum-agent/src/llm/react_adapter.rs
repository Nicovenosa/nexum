...
impl ReactLLM for BaseModelReactLLM {
    async fn generate_reasoning(
        ...
        if response.stop_reason == StopReason::ToolUse {
            ...
            // Antes de dar esto por respuesta final: ¿el modelo escribió un tool
            // call como texto? Los modelos locales chicos emiten llamadas bien
            // formadas envueltas en un bloque markdown, y sin destaparlo se
            // pierden. Sé liberal al aceptar; el filtro por tool disponible es
            // lo que mantiene la parte estricta.
            let nombres: Vec<&str> = tools.iter().map(|t| t.name()).collect();
            let recuperados =
                crate::llm::tool_call_recovery::recover_tool_calls(&text, &nombres);
            if !recuperados.is_empty() {
                tracing::info!(
                    model = %model_name,
                    tool_count = recuperados.len(),
                    tools = ?recuperados.iter().map(|c| c.name.as_str()).collect::<Vec<_>>(),
                    "tool call recuperado del texto: el modelo lo emitió envuelto en markdown"
                );
                let calls: Vec<ToolCall> = recuperados
                    .into_iter()
                    .enumerate()
                    .map(|(i, c)| {
                        ToolCall::new(format!("recovered_{i}"), c.name, c.arguments)
                    })
                    .collect();
                let r = Reasoning::with_tools(String::new(), calls);
                return Ok(Self::finalize_reasoning(
                    r, response, model_name, streamed, usage,
                ));
            }
