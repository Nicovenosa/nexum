...
pub(super) async fn do_invoke_streaming(
    ...
    let resp = adapter
        .client
        .post(&chat_url)
        // Ver invoke.rs: sin credencial no se manda Authorization.
        .bearer_auth_if_present(&adapter.api_key)
        .json(&body)
        .send()
        .await
        .map_err(|e| {
    ...
    loop {
        ...
        let chunk = tokio::select! {
            ...
            result = stream.next() => {
                match result {
                    Some(Ok(c)) => c,
                    Some(Err(e)) => return Err(AgentError::from_reqwest(e)),
                    None => break,
                }
            }
        };
    ...
    validate_stream_terminal(parser.is_done())?;
...
/// OpenAI-compatible SSE requires the explicit `[DONE]` marker. Returning
/// accumulated text after transport EOF would turn a truncated response into
/// a false success.
fn validate_stream_terminal(done: bool) -> Result<(), AgentError> {
    if done {
        Ok(())
    } else {
        Err(AgentError::LlmError(
            "EOF_WITHOUT_FINAL: OpenAI SSE closed before [DONE]".to_string(),
        ))
    }
}
...
mod terminal_tests {
    ...
    #[test]
    fn eof_without_done_is_failed_not_partial_success() {
        let error = validate_stream_terminal(false).unwrap_err();
        assert!(error.to_string().contains("EOF_WITHOUT_FINAL"));
        assert!(validate_stream_terminal(true).is_ok());
    }
}
