...
impl BaseModel for ChatOpenAI {
    async fn invoke(&self, request: LlmRequest) -> AgentResult<LlmResponse> {
        ...
        let resp = self
            .client
            .post(&chat_url)
            // Sin credencial NO se manda Authorization: el tier libre de
            // OpenCode rechaza un Bearer vacío o de relleno con 401.
            // Con api_key presente el comportamiento es idéntico al de antes.
    ...
    async fn invoke_streaming(
        ...
        // Capability-aware: si este provider tiene streaming deshabilitado
        // (p. ej. Ollama local vía with_local_cpu_profile), degradar al camino
        // non-stream en vez de abrir un SSE. Preserva tool_calls/finish_reason
        // porque invoke() los parsea igual.
        if !self.streaming_enabled {
            tracing::debug!(
                provider = "openai",
                model = %self.model,
                "streaming deshabilitado (capability-aware) — invoke_streaming degrada a invoke() non-stream"
            );
