...
pub(super) async fn do_invoke_streaming(
    ...
    validate_stream_terminal(message_stopped)?;
...
/// Anthropic uses `message_stop` rather than OpenAI's `[DONE]`. EOF is only a
/// valid transport close after that provider-specific final event.
fn validate_stream_terminal(message_stopped: bool) -> Result<(), AgentError> {
    if message_stopped {
        Ok(())
    } else {
        Err(AgentError::LlmError(
            "EOF_WITHOUT_FINAL: Anthropic SSE closed before message_stop".to_string(),
        ))
    }
}
...
mod terminal_tests {
    ...
    #[test]
    fn eof_without_message_stop_is_failed_not_partial_success() {
