...
struct MetricEvent {
    /// ISO 8601 毫秒时间戳
    /// Epoch ms. Ver crate::jsonl para el esquema único.
    ts: u64,
    /// session_id
...
pub fn emit(event: &str, data: serde_json::Value, sid: Option<&str>, rid: Option<&str>) {
    ...
    let evt = MetricEvent {
        // Epoch ms, igual que turn_log: ordenar y restar tiene que ser trivial.
        // Antes acá iba RFC3339 y en el otro sink un entero, en el MISMO archivo.
        ts: crate::jsonl::now_millis() as u64,
...
async fn metrics_writer(mut rx: mpsc::UnboundedReceiver<MetricEvent>) {
    // Ruta por la FUENTE ÚNICA, no construida acá: los dos sinks escriben al
    // mismo archivo y resolvían por separado. Además hereda el aislamiento
    // fail-closed bajo test, que es lo que faltaba cuando una corrida de la
    // suite contaminó la evidencia de un experimento.
    let Some(base_dir) = crate::turn_log::metrics_dir() else {
        tracing::warn!("no se pudo resolver el directorio de metrics");
        return;
    };
