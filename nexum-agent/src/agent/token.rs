...
impl TokenTracker {
    ...
    /// Eficiencia de cache de la última llamada. `None` SIN MUESTRAS.
    ///
    /// Devolvía 0.0 sin denominador, y eso hacía que cualquier umbral
    /// `rate < piso` disparara con cero llamadas: "no sé" se leía como "está
    /// mal". `Option` lo vuelve imposible de confundir, que es como ya lo
    /// resuelven `usage_pct` y `context_usage_percent`.
...
impl RequestRecord {
    ...
    /// Eficiencia de cache de este request. `None` SIN MUESTRAS: ver arriba.
    pub fn cache_hit_rate(&self) -> Option<f64> {
        if self.input_tokens == 0 {
            return None;
        }
        Some(self.cache_read_input_tokens as f64 / self.input_tokens as f64)
    }
}
