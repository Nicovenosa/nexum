...
/// Tool que sincroniza con una barrier compartida: solo la libera si OTRA
/// tool está esperando la misma barrier al mismo tiempo. Prueba paralelismo de
/// forma DETERMINÍSTICA (sin depender de wall-clock): si el executor corriera
/// las tools en serie, la primera esperaría la barrier para siempre y el test
/// haría timeout. Reemplaza al viejo umbral frágil de 160ms.
struct SlowTool {
...
impl BaseTool for SlowTool {
    ...
    async fn invoke(
        ...
        // Solo avanza cuando la otra tool también llegó (paralelismo real).
...
async fn test_parallel_tool_execution() {
    // Barrier de 2: las dos tools solo se liberan si ambas están ejecutándose
    // a la vez. Si el executor las corriera en serie, la barrier nunca se
    // completa y el `timeout` de abajo dispara → el test falla de forma
    // determinística (sin umbral de wall-clock frágil).
    ...
    let output = tokio::time::timeout(
        ...
        agent.execute(AgentInput::text("go"), &mut state, None),
    )
    .await
    .expect("las tools corren en paralelo (barrier se completa); en serie haría timeout")
    .unwrap();
