...
#[test]
fn test_cache_hit_rate_none_sin_muestras_y_some_cero_con_muestras() {
    // SIN llamadas: no hay denominador. `None`, no 0.0 — la distinción es el
    // punto: 0.0 hacía disparar cualquier umbral `rate < piso` con cero datos.
    let tracker = TokenTracker::default();
    assert_eq!(tracker.cache_hit_rate(), None, "cero muestras no es una tasa");

    // CON una llamada y cache ausente (API OpenAI-compatible): 0% real.
    let mut tracker2 = TokenTracker::default();
    tracker2.accumulate(&make_usage(1000, 500, None, None));
    assert_eq!(
        tracker2.cache_hit_rate(),
        Some(0.0),
        "una llamada sin cache SÍ es un 0% medido"
    );
}
...
fn test_cache_hit_rate_zero_on_first_creation() {
    ...
    assert_eq!(tracker.cache_hit_rate(), Some(0.0), "hay muestra: 0% medido");
}
...
fn test_cache_hit_rate_reflects_latest_call() {
    ...
    assert_eq!(tracker.cache_hit_rate(), Some(0.0));

    // 第二次调用：高缓存命中 34230/34820 ≈ 98.3%
    tracker.accumulate(&make_usage(34820, 423, None, Some(34230)));
    let rate = tracker.cache_hit_rate().expect("hay muestras");
    assert!(
...
fn test_cache_hit_rate_after_reset() {
    ...
    tracker.reset();
    assert_eq!(tracker.cache_hit_rate(), None, "tras reset no hay muestras");
}
...
fn test_cache_hit_rate_openai_pattern() {
    ...
    tracker.accumulate(&make_usage(10000, 500, None, Some(0)));
    assert_eq!(tracker.cache_hit_rate(), Some(0.0), "cache_read=0 应返回 0.0");
...
fn test_request_record_cache_hit_rate() {
    ...
    assert_eq!(record.cache_hit_rate(), Some(0.0));
    ...
    assert!((record2.cache_hit_rate().expect("hay muestras") - 8000.0 / 8500.0).abs() < 1e-9);
}
