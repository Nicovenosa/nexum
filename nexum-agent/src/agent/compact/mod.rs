...
///
/// 作为单一事实源，由三条路径共享：
/// - `/compact` 命令路径（`nexum-acp/src/session/command/compact/invariant.rs`）
/// - 自动 compact 路径（`nexum-middlewares/src/compact_middleware.rs`）
/// - TUI 识别层（`nexum-tui/src/ui/message_view/build.rs::COMPACT_HINT`）
///
