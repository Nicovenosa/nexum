...
impl SqliteThreadStore {
    ...
    /// Fallback cuando `default_path` no se puede abrir.
    ///
    /// # Por qué NO va en /tmp y NO va por PID
    ///
    /// Esta base guarda el HISTORIAL DE CONVERSACIONES del usuario: es dato
    /// persistente suyo, no estado de sesión. Las dos alternativas obvias están
    /// las dos mal, y por razones distintas:
    ///
    /// - **`/tmp/<nombre-fijo>`** era lo que había, y es un bug de producción:
    ///   dos instancias de `nexum` a la vez comparten la MISMA SQLite, con dos
    ///   escritores sobre las mismas tablas. Y `/tmp` es volátil, así que el
    ///   historial se pierde al reiniciar sin que nadie avise. Degradar en
    ///   silencio a un lugar que se borra es peor que fallar.
    /// - **Por PID** arreglaría la colisión y rompería la función: cada arranque
    ///   vería un historial vacío, y el usuario perdería sus conversaciones cada
    ///   vez. El aislamiento por PID es correcto para estado de sesión —sockets,
    ///   métricas de test— y equivocado para dato del usuario.
    ///
    /// Va bajo el home de Nexum, con un nombre distinto del principal para que
    /// se vea que es una degradación y no la ruta normal. Si tampoco se puede
    /// abrir, el error se propaga: el llamador decide, no se inventa un lugar.
    pub async fn fallback_path() -> Result<Self> {
        let db_path = crate::config_home::nexum_home()
            .join("threads")
            .join("threads-fallback.db");
        if let Some(parent) = db_path.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        tracing::warn!(
            path = %db_path.display(),
            "la base de threads principal no se pudo abrir; usando fallback"
        );
